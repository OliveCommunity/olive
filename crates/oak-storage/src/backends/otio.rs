...
use oak_node::block::{ClipBlockBehavior, GapBlockBehavior, TransitionBlockBehavior};
...
impl OtioBackend {
	...
	pub fn save_project(
		...
		match ext.as_str() {
			...
			"fcpxml" => oak_otio::to_fcpxml_file(&timelines, &path)
	...
	fn load(&self, uri: &StorageUri) -> Result<LoadResult> {
		...
		let timelines: Vec<Timeline> = match ext.as_str() {
			"otio" => {
				...
				let root = oak_otio::from_json_string(&text)
			...
			"fcpxml" => {
				oak_otio::from_fcpxml_file(&path).map_err(|e| Error::Format(e.to_string()))?
			}
...
fn export_track(graph: &oak_node::graph::Graph, track: &TrackBehavior, fps: f64) -> Composable {
	...
	for block_id in &track.blocks {
		...
		match kind {
			BlockKind::Clip => {
				...
				let mut clip = oak_otio::model::Clip::new(clip_name(graph, footage));
...
fn clip_name(graph: &oak_node::graph::Graph, footage: Option<NodeId>) -> String {
	match footage {
		Some(fid) => {
...
fn classify_block(
	entry: &NodeEntry,
) -> (BlockKind, Option<&oak_node::block::BlockCore>) {
	entry
		.behavior
		.as_any()
		.and_then(|a| {
...
fn to_rt(r: Rational, fps: f64) -> oak_otio::model::RationalTime {
	oak_otio::model::RationalTime::from_rational(r, fps)
}
			...
			match child.as_ref() {
				Composable::Clip(c) => {
					...
					let block_id = {
						...
						clip.core.range = oak_core::TimeRange::new(pos, pos + len);
				...
				Composable::Gap(g) => {
					...
					let block_id = {
						...
						gap.core.range = oak_core::TimeRange::new(pos, pos + len);
				...
				Composable::Transition(t) => {
					...
					let block_id = {
						...
						tr.core.range =
							oak_core::TimeRange::new(pos, pos + Rational::new(1, 1));
