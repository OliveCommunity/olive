...
impl TrackBehavior {
	...
	/// `is_range_free`).
	pub fn is_range_free(&self, range: oak_core::TimeRange, blocks: &dyn BlockRange) -> bool {
		!self.blocks.iter().any(|b| blocks.overlaps(*b, range))
	}

	/// Total length (end of the last block; C++ `Track::get_length`).
	pub fn length(&self, blocks: &dyn BlockRange) -> oak_core::Rational {
		let mut end = oak_core::Rational::new(0, 1);
...
pub trait BlockRange {
	/// In-point of `block`.
	fn in_(&self, block: NodeId) -> oak_core::Rational;
	/// Out-point of `block`.
	fn out(&self, block: NodeId) -> oak_core::Rational;
	/// True when `time` is strictly inside the block.
	fn contains_strict(&self, block: NodeId, time: oak_core::Rational) -> bool {
		let (in_, out) = (self.in_(block), self.out(block));
		time > in_ && time < out
	}
	/// True when `time` is visible on the block (in <= t < out).
	fn contains(&self, block: NodeId, time: oak_core::Rational) -> bool {
		let (in_, out) = (self.in_(block), self.out(block));
		time >= in_ && time < out
	}
	/// True when the block's span overlaps `range`.
	fn overlaps(&self, block: NodeId, range: oak_core::TimeRange) -> bool {
		let (in_, out) = (self.in_(block), self.out(block));
...
impl TrackListBehavior {
	...
	pub fn total_length(&self, tracks: &dyn TrackRange) -> oak_core::Rational {
...
pub trait TrackRange {
	...
	fn length(&self, track: NodeId) -> oak_core::Rational;
}
