...
use oaknode::ffi::block::{
	oaknode_block_as_node, oaknode_block_clip_create, oaknode_block_free, oaknode_block_from_node,
	...
	oaknode_block_set_out, oaknode_clip_get_media_in, oaknode_clip_get_speed,
	oaknode_clip_set_media_in, oaknode_clip_set_speed, oaknode_transition_get_in_offset,
...
use oaknode::ffi::colormanager::{
	...
	oaknode_colormanager_init, oaknode_colormanager_initialize,
...
use oaknode::ffi::group::{
	...
	oaknode_group_cast, oaknode_group_create, oaknode_group_free,
	oaknode_group_get_output_passthrough, oaknode_group_passthrough_count,
	oaknode_group_passthrough_id_at, oaknode_group_passthrough_input_at,
	oaknode_group_remove_input_passthrough, oaknode_group_resolve_input,
	oaknode_group_set_output_passthrough, oaknode_group_set_output_passthrough_undoable,
};
use oaknode::ffi::keyframe::{
	...
	oaknode_keyframe_get_value, oaknode_keyframe_get_value_string,
	oaknode_keyframe_has_sibling_at_time, oaknode_keyframe_opposing_bezier_type,
	oaknode_keyframe_set_bezier_control, oaknode_keyframe_set_bezier_control_undoable,
	oaknode_keyframe_set_time, oaknode_keyframe_set_time_undoable, oaknode_keyframe_set_type,
	oaknode_keyframe_set_type_undoable, oaknode_keyframe_set_value,
	oaknode_keyframe_set_value_string, oaknode_keyframe_set_value_string_undoable,
...
use oaknode::ffi::serializer::{
	oaknode_serializer_initialize, oaknode_serializer_load_from_file,
	oaknode_serializer_load_from_xml, oaknode_serializer_loaddata_free,
...
use oaknode::ffi::track::{
	oaknode_track_append_block, oaknode_track_as_node, oaknode_track_create, oaknode_track_free,
	oaknode_track_get_block_at, oaknode_track_get_block_count, oaknode_track_get_height,
	oaknode_track_get_index, oaknode_track_get_length, oaknode_track_get_muted,
	oaknode_track_get_reference, oaknode_track_get_type, oaknode_track_prepend_block,
	oaknode_track_ripple_remove_block, oaknode_track_set_height, oaknode_track_set_index,
	oaknode_track_set_muted, oaknode_tracklist_add_track, oaknode_tracklist_array_append,
...
fn folder_family() {
	...
	assert!(unsafe { oaknode_folder_create(CHandle::null()) }
		.ctx
		.is_null());
	...
	assert!(unsafe { oaknode_folder_child_at(dup(&root), 5) }
		.ctx
		.is_null());
	assert_eq!(
		unsafe { oaknode_folder_index_of_child(dup(&root), dup(&child)) },
		0
	);
	assert_eq!(
	...
	assert!(unsafe { oaknode_folder_parent_of(dup(&folder)) }
		.ctx
		.is_null());
...
fn sequence_family() {
	...
	let mut vc = 0;
	assert_eq!(
		unsafe { oaknode_sequence_get_audio_stream_count(dup(&seq), &mut vc) },
		OAKNODE_OK
	);
	assert_eq!(vc, 1);
	...
	assert_eq!(
		unsafe { oaknode_sequence_get_track_list(dup(&seq), 0, &mut list) },
		OAKNODE_OK
	);
	...
	let mut tc = 0;
	assert_eq!(
		unsafe { oaknode_sequence_get_track_count(dup(&seq), 0, &mut tc) },
		OAKNODE_OK
	);
	assert_eq!(tc, 0);
	...
	// Playhead round-trip.
	assert_eq!(
		unsafe { oaknode_sequence_set_playhead(dup(&seq), 30, 1) },
		OAKNODE_OK
	);
	let mut num = 0;
	let mut den = 0;
	assert_eq!(
		unsafe { oaknode_sequence_get_playhead(dup(&seq), &mut num, &mut den) },
		OAKNODE_OK
	);
	...
	assert_eq!(
		unsafe { oaknode_sequence_verify_length(dup(&seq)) },
		OAKNODE_OK
	);

	// Failure paths on empty handles.
	assert_eq!(
		unsafe { oaknode_sequence_get_playhead(CHandle::null(), &mut num, &mut den) },
		OAKNODE_E_INVALID
	);
	assert!(unsafe { oaknode_sequence_from_node(CHandle::null()) }
		.ctx
		.is_null());
...
fn track_family() {
	...
	let mut ty = -1;
	assert_eq!(
		unsafe { oaknode_track_get_type(dup(&track), &mut ty) },
		OAKNODE_OK
	);
	assert_eq!(ty, 0);
	...
	let mut h = 0.0;
	assert_eq!(
		unsafe { oaknode_track_get_height(dup(&track), &mut h) },
		OAKNODE_OK
	);
	assert!((h - 3.0).abs() < 1e-9);
	assert_eq!(
		unsafe { oaknode_track_set_height(dup(&track), 5.0) },
		OAKNODE_OK
	);
	assert_eq!(
		unsafe { oaknode_track_get_height(dup(&track), &mut h) },
	...
	// Muted/locked/index.
	assert_eq!(
		unsafe { oaknode_track_set_muted(dup(&track), 1) },
		OAKNODE_OK
	);
	let mut muted = 0;
	assert_eq!(
		unsafe { oaknode_track_get_muted(dup(&track), &mut muted) },
		OAKNODE_OK
	);
	assert_eq!(muted, 1);
	assert_eq!(
		unsafe { oaknode_track_set_index(dup(&track), 2) },
		OAKNODE_OK
	);
	let mut idx = 0;
	assert_eq!(
		unsafe { oaknode_track_get_index(dup(&track), &mut idx) },
		OAKNODE_OK
	);
	assert_eq!(idx, 2);
	let mut rty = 0;
	let mut ridx = 0;
	assert_eq!(
		unsafe { oaknode_track_get_reference(dup(&track), &mut rty, &mut ridx) },
		OAKNODE_OK
	);
	...
	assert_eq!(
		unsafe { oaknode_track_append_block(dup(&track), dup(&clip1)) },
		OAKNODE_OK
	);
	assert_eq!(
		unsafe { oaknode_track_append_block(dup(&track), dup(&clip2)) },
		OAKNODE_OK
	);
	assert_eq!(
		unsafe { oaknode_track_prepend_block(dup(&track), dup(&clip3)) },
		OAKNODE_OK
	);
	let mut bc = 0;
	assert_eq!(
		unsafe { oaknode_track_get_block_count(dup(&track), &mut bc) },
		OAKNODE_OK
	);
	assert_eq!(bc, 3);
	let mut blk = CHandle::null();
	assert_eq!(
		unsafe { oaknode_track_get_block_at(dup(&track), 0, &mut blk) },
		OAKNODE_OK
	);
	assert!(!blk.ctx.is_null());
	assert_eq!(
		unsafe { oaknode_track_get_block_at(dup(&track), 5, &mut blk) },
		OAKNODE_E_NOT_FOUND
	);
	assert_eq!(
		unsafe { oaknode_track_ripple_remove_block(dup(&track), dup(&clip1)) },
		OAKNODE_OK
	);
	assert_eq!(
		unsafe { oaknode_track_get_block_count(dup(&track), &mut bc) },
	...
	let mut len_den = 0;
	assert_eq!(
		unsafe { oaknode_track_get_length(dup(&track), &mut len_num, &mut len_den) },
		OAKNODE_OK
	);
	...
	assert_eq!(
		unsafe { oaknode_sequence_get_track_list(dup(&seq), 0, &mut list2) },
		OAKNODE_OK
	);
	let mut lt = 0;
	assert_eq!(
		unsafe { oaknode_tracklist_get_type(dup(&list2), &mut lt) },
		OAKNODE_OK
	);
	assert_eq!(lt, 0);
	let input_id =
		two_stage(|b, s| unsafe { oaknode_tracklist_get_track_input_id(dup(&list2), b, s) })
			.unwrap();
	assert_eq!(input_id, "track_in_0");
	assert_eq!(
		unsafe { oaknode_tracklist_array_append(dup(&list2)) },
		OAKNODE_OK
	);
	let mut asz = 0;
	assert_eq!(
		unsafe { oaknode_tracklist_get_array_size(dup(&list2), &mut asz) },
		OAKNODE_OK
	);
	assert_eq!(asz, 1);
	...
	assert_eq!(
		unsafe { oaknode_tracklist_add_track(dup(&list2), dup(&track)) },
		OAKNODE_OK
	);
	let mut tcount = 0;
	assert_eq!(
		unsafe { oaknode_tracklist_get_track_count(dup(&list2), &mut tcount) },
		OAKNODE_OK
	);
	assert_eq!(tcount, 1);
	let mut track_at = CHandle::null();
	assert_eq!(
		unsafe { oaknode_tracklist_get_track_at(dup(&list2), 0, &mut track_at) },
		OAKNODE_OK
	);
	assert!(!track_at.ctx.is_null());
	assert_eq!(
		unsafe { oaknode_tracklist_remove_track(dup(&list2), dup(&track)) },
		OAKNODE_OK
	);
	assert_eq!(
		unsafe { oaknode_tracklist_remove_track(dup(&list2), dup(&track)) },
...
fn block_family() {
	...
	let mut kind = 0;
	assert_eq!(
		unsafe { oaknode_block_get_kind(dup(&clip), &mut kind) },
		OAKNODE_OK
	);
	assert_eq!(kind, 1, "clip kind");
	assert_eq!(
		unsafe { oaknode_block_get_kind(dup(&gap), &mut kind) },
		OAKNODE_OK
	);
	...
	assert!(!back.ctx.is_null());
	assert!(unsafe { oaknode_block_from_node(CHandle::null()) }
		.ctx
		.is_null());
	...
	let mut len_den = 0;
	assert_eq!(
		unsafe { oaknode_block_get_length(dup(&clip), &mut len_num, &mut len_den) },
		OAKNODE_OK
	);
	assert_eq!((len_num, len_den), (1, 1));
	assert_eq!(
		unsafe { oaknode_block_set_out(dup(&clip), 5, 1) },
		OAKNODE_OK
	);
	assert_eq!(
		unsafe { oaknode_block_get_out(dup(&clip), &mut len_num, &mut len_den) },
		OAKNODE_OK
	);
	...
	let mut en = 0;
	assert_eq!(
		unsafe { oaknode_block_get_enabled(dup(&clip), &mut en) },
		OAKNODE_OK
	);
	assert_eq!(en, 1);
	assert_eq!(
		unsafe { oaknode_block_set_enabled(dup(&clip), 0) },
		OAKNODE_OK
	);
	assert_eq!(
		unsafe { oaknode_block_get_enabled(dup(&clip), &mut en) },
	...
	let mut mden = 0;
	assert_eq!(
		unsafe { oaknode_clip_get_media_in(dup(&clip), &mut min, &mut mden) },
		OAKNODE_OK
	);
	assert_eq!((min, mden), (0, 1));
	assert_eq!(
		unsafe { oaknode_clip_set_media_in(dup(&clip), 10, 1) },
		OAKNODE_OK
	);
	assert_eq!(
		unsafe { oaknode_clip_get_media_in(dup(&clip), &mut min, &mut mden) },
	...
	let mut speed = 0.0;
	assert_eq!(
		unsafe { oaknode_clip_get_speed(dup(&clip), &mut speed) },
		OAKNODE_OK
	);
	assert_eq!(speed, 1.0);
	assert_eq!(
		unsafe { oaknode_clip_set_speed(dup(&clip), 2.0) },
		OAKNODE_OK
	);
	assert_eq!(
		unsafe { oaknode_clip_get_speed(dup(&clip), &mut speed) },
	...
	// Failure paths.
	assert_eq!(
		unsafe { oaknode_block_get_length(CHandle::null(), &mut len_num, &mut len_den) },
		OAKNODE_E_INVALID
	);
	assert_eq!(
...
fn footage_family() {
	...
	let mut footage = unsafe { oaknode_footage_create(dup(&p), cs("/tmp/media.mov").as_ptr()) };
	assert!(!footage.ctx.is_null());
	assert!(
		unsafe { oaknode_footage_create(CHandle::null(), cs("x").as_ptr()) }
			.ctx
			.is_null()
	);
	...
	// Proxy fields.
	assert_eq!(
		unsafe { oaknode_footage_set_proxy(dup(&footage), cs("/proxy.mov").as_ptr(), 2, 0, 1, 1,) },
		OAKNODE_OK
	);
	let proxy =
		two_stage(|b, s| unsafe { oaknode_footage_proxy_path(dup(&footage), b, s) }).unwrap();
	assert_eq!(proxy, "/proxy.mov");
	assert_eq!(
		unsafe { oaknode_footage_clear_proxy(dup(&footage)) },
		OAKNODE_OK
	);
	let proxy =
		two_stage(|b, s| unsafe { oaknode_footage_proxy_path(dup(&footage), b, s) }).unwrap();
...
fn colormanager_family() {
	...
	assert_eq!(
		unsafe {
			oaknode_colormanager_get_config_filename(CHandle::null(), std::ptr::null_mut(), 0)
		},
		OAKNODE_E_INVALID
	);
	...
	// oakrender).
	assert_eq!(
		unsafe { oaknode_colormanager_initialize(dup(&cm)) },
		OAKNODE_OK
	);
	assert_eq!(
		unsafe { oaknode_colormanager_get_display_count(dup(&cm), &mut count) },
...
fn traverser_family() {
	...
	let mut rows = 0;
	assert_eq!(
		unsafe { oaknode_traverser_database_row_count(dup(&db), &mut rows) },
		OAKNODE_OK
	);
...
fn serializer_family() {
	...
	let mut count = 0;
	assert_eq!(
		unsafe { oaknode_serializer_loaddata_node_count(dup(&ld)) },
		1
	);
	let _ = count;
	...
	assert!(unsafe { oaknode_serializer_loaddata_node_at(dup(&ld), 5) }
		.ctx
		.is_null());
	...
	let path =
		std::env::temp_dir().join(format!("oaknode-serializer-{}.oakproj", std::process::id()));
...
fn keyframe_family_basic() {
	...
	assert!(unsafe {
		oaknode_keyframe_create(0, 1, &sv, 0, 0, -1, cs("param_a_in").as_ptr(), dup(&math))
	}
	.ctx
	.is_null());
	...
	assert_eq!(
		unsafe { oaknode_keyframe_get_time(dup(&kf), &mut num, &mut den) },
		OAKNODE_OK
	);
	...
	assert_eq!(
		unsafe { oaknode_keyframe_set_time(dup(&kf), 2, 25) },
		OAKNODE_OK
	);
	assert_eq!(
		unsafe { oaknode_keyframe_get_time(dup(&kf), &mut num, &mut den) },
	...
	assert_eq!(
		unsafe { oaknode_keyframe_get_value(dup(&kf), &mut out) },
		OAKNODE_OK
	);
	...
	assert_eq!(
		unsafe { oaknode_keyframe_set_value(dup(&kf), &f(7.0)) },
		OAKNODE_OK
	);
	assert_eq!(
		unsafe { oaknode_keyframe_get_value(dup(&kf), &mut out) },
	...
	let mut ty = -1;
	assert_eq!(
		unsafe { oaknode_keyframe_get_type(dup(&kf), &mut ty) },
		OAKNODE_OK
	);
	assert_eq!(ty, 0, "linear");
	assert_eq!(
		unsafe { oaknode_keyframe_set_type(dup(&kf), 2) },
		OAKNODE_OK
	);
	assert_eq!(
		unsafe { oaknode_keyframe_get_type(dup(&kf), &mut ty) },
	...
	assert_eq!(
		unsafe { oaknode_keyframe_get_bezier_control(dup(&kf), 0, &mut bx, &mut by) },
		OAKNODE_OK
	);
	assert_eq!((bx, by), (-1.0, 0.0), "default in-handle");
	assert_eq!(
		unsafe { oaknode_keyframe_get_bezier_control(dup(&kf), 1, &mut bx, &mut by) },
		OAKNODE_OK
	);
	...
	assert_eq!(
		unsafe { oaknode_keyframe_set_bezier_control(dup(&kf), 0, -0.4, 1.2) },
		OAKNODE_OK
	);
	assert_eq!(
	...
	assert_eq!(
		(bx, by),
		(-0.4, 1.2),
		"standalone handles are already valid"
	);

	// track / element / input / parent.
	let mut trk = -1;
	assert_eq!(
		unsafe { oaknode_keyframe_get_track(dup(&kf), &mut trk) },
		OAKNODE_OK
	);
	assert_eq!(trk, 0);
	let mut el = -9;
	assert_eq!(
		unsafe { oaknode_keyframe_get_element(dup(&kf), &mut el) },
		OAKNODE_OK
	);
	assert_eq!(el, -1);
	...
	assert_eq!(
		unsafe { oaknode_keyframe_get_parent(dup(&kf), &mut parent) },
		OAKNODE_OK
	);
	assert!(!parent.ctx.is_null());
	...
	assert_eq!(
		unsafe { oaknode_keyframe_get_parent(dup(&kf2), &mut parent2) },
		OAKNODE_OK
	);
	...
	assert_eq!(
		unsafe { oaknode_keyframe_set_value_string(dup(&kf), cs("hello").as_ptr()) },
		OAKNODE_OK
	);
	let s =
		two_stage(|b, sz| unsafe { oaknode_keyframe_get_value_string(dup(&kf), b, sz) }).unwrap();
	assert_eq!(s, "hello");
	...
	assert_eq!(
		unsafe { oaknode_keyframe_opposing_bezier_type(7) },
		OAKNODE_E_INVALID
	);
	...
	let mut sib = -1;
	assert_eq!(
		unsafe { oaknode_keyframe_has_sibling_at_time(dup(&kf), 1, 25, &mut sib) },
		OAKNODE_OK
	);
	assert_eq!(sib, 0);
	assert_eq!(
	...
	assert_eq!(
		unsafe { oaknode_keyframe_compute_paste_value(dup(&math), dup(&kf), &mut pv) },
		OAKNODE_OK
	);
...
fn keyframe_undoable() {
	...
	assert_eq!(
		unsafe { oaknode_keyframe_set_time_undoable(dup(&kf), 5, 1, &mut cmd) },
		OAKNODE_OK
	);
	...
	let mut cmd = CHandle::null();
	assert_eq!(
		unsafe { oaknode_keyframe_set_value_undoable(dup(&kf), &f(9.0), &mut cmd) },
		OAKNODE_OK
	);
	assert_eq!(unsafe { oakundo_command_redo_now(dup(&cmd)) }.unwrap(), 0);
	let mut out = OakNodeValue::none();
	...
	let mut cmd = CHandle::null();
	assert_eq!(
		unsafe { oaknode_keyframe_set_type_undoable(dup(&kf), 1, &mut cmd) },
		OAKNODE_OK
	);
	assert_eq!(unsafe { oakundo_command_redo_now(dup(&cmd)) }.unwrap(), 0);
	let mut ty = -1;
	...
	assert_eq!(
		unsafe {
			oaknode_keyframe_set_value_string_undoable(dup(&kf), cs("str").as_ptr(), &mut cmd)
		},
		OAKNODE_OK
	);
	assert_eq!(unsafe { oakundo_command_redo_now(dup(&cmd)) }.unwrap(), 0);
...
fn group_family_basic() {
	...
	assert_eq!(
		unsafe { oaknode_project_add_node(dup(&p), dup(&group)) },
		OAKNODE_OK
	);
	let mut math = unsafe {
		oaknode::ffi::factory::oaknode_factory_create_from_id(
	...
	assert_eq!(
		unsafe { oaknode_project_add_node(dup(&p2), dup(&math2)) },
		OAKNODE_OK
	);
	assert_eq!(
		unsafe {
	...
	let mut count = 0;
	assert_eq!(
		unsafe { oaknode_group_passthrough_count(dup(&group), &mut count) },
		OAKNODE_OK
	);
	assert_eq!(count, 1);
	let pid =
		two_stage(|b, s| unsafe { oaknode_group_passthrough_id_at(dup(&group), 0, b, s) }).unwrap();
	...
	assert_eq!(
		unsafe { oaknode_group_set_output_passthrough(dup(&group), dup(&math)) },
		OAKNODE_OK
	);
	let mut out = CHandle::null();
	assert_eq!(
		unsafe { oaknode_group_get_output_passthrough(dup(&group), &mut out) },
		OAKNODE_OK
	);
...
fn dragger_family() {
	...
	assert!(
		unsafe { oaknode_dragger_create(dup(&math), cs("nope_in").as_ptr(), -1, 0) }
			.ctx
			.is_null()
	);
	...
	let mut started = -1;
	assert_eq!(
		unsafe { oaknode_dragger_is_started(dup(&dg), &mut started) },
		OAKNODE_OK
	);
	assert_eq!(started, 0);
	...
	assert_eq!(
		unsafe { oaknode_dragger_start(dup(&dg2), 0, 1, -1, 1) },
		OAKNODE_E_INVALID
	);

	// start at time 0 on track 0; double start -> E_STATE.
	assert_eq!(
		unsafe { oaknode_dragger_start(dup(&dg), 0, 1, 0, 1) },
		OAKNODE_OK
	);
	assert_eq!(
		unsafe { oaknode_dragger_is_started(dup(&dg), &mut started) },
	...
	assert_eq!(started, 1);
	assert_eq!(
		unsafe { oaknode_dragger_start(dup(&dg), 1, 1, 0, 1) },
		OAKNODE_E_STATE
	);
	...
	assert_eq!(
		unsafe { oaknode_dragger_drag(dup(&dg), &f(4.0)) },
		OAKNODE_OK
	);
	let mut out = OakNodeValue::none();
	assert_eq!(
		unsafe {
			oaknode_node_get_input_at_time(dup(&math), cs("param_a_in").as_ptr(), 0, 1, &mut out)
		},
		OAKNODE_OK
	);
	assert_eq!(out.kind, oak::FLOAT);
	assert_eq!(out.f[0], 4.0);
	let bad = OakNodeValue {
		kind: oak::INT,
		num: 3,
		den: 0,
		f: [0.0; 4],
	...
	assert_eq!(
		unsafe { oaknode_dragger_drag(dup(&dg), &bad) },
		OAKNODE_E_INVALID
	);
	// drag before start -> E_STATE.
	assert_eq!(
		unsafe { oaknode_dragger_drag(dup(&dg2), &f(1.0)) },
		OAKNODE_E_STATE
	);
	...
	let mut cmd = CHandle::null();
	assert_eq!(
		unsafe { oaknode_dragger_end(dup(&dg), &mut cmd) },
		OAKNODE_OK
	);
	assert!(!cmd.ctx.is_null());
	assert_eq!(
	...
	// Null args -> E_INVALID.
	assert_eq!(
		unsafe { oaknode_dragger_is_started(CHandle::null(), &mut started) },
		OAKNODE_E_INVALID
	);
	assert_eq!(
		unsafe { oaknode_dragger_drag(CHandle::null(), &f(1.0)) },
		OAKNODE_E_INVALID
	);
...
fn undoable_node_exports() {
	...
	assert_eq!(unsafe { oakundo_command_redo_now(dup(&cmd)) }.unwrap(), 0);
	let label =
		two_stage(|b, s| unsafe { oaknode::ffi::node::oaknode_node_get_label(dup(&math), b, s) })
			.unwrap();
	...
	let mut en = 1;
	assert_eq!(
		unsafe { oaknode::ffi::node::oaknode_node_is_enabled(dup(&math), &mut en) },
		OAKNODE_OK
	);
	assert_eq!(en, 1);
	assert_eq!(unsafe { oakundo_command_redo_now(dup(&cmd)) }.unwrap(), 0);
	...
	assert_eq!(
		unsafe {
			oaknode_node_set_input_undoable(dup(&math), cs("param_a_in").as_ptr(), &v, &mut cmd)
		},
		OAKNODE_OK
	);
	assert_eq!(unsafe { oakundo_command_redo_now(dup(&cmd)) }.unwrap(), 0);
	...
	assert_eq!(
		unsafe { oaknode_project_add_node(dup(&p), dup(&other)) },
		OAKNODE_OK
	);
	let mut cmd = CHandle::null();
	assert_eq!(
	...
	assert_eq!(
		unsafe {
			oaknode_node_disconnect_undoable(dup(&other), cs("param_a_in").as_ptr(), &mut cmd)
		},
		OAKNODE_E_NOT_FOUND
	);
...
impl oaknode::node::NodeBehavior for FakeMulticam {
	...
	fn duplicate(
		&self,
		_core: &oaknode::node::NodeCore,
	) -> Option<Box<dyn oaknode::node::NodeBehavior>> {
		None
	}
}
...
fn multicam_family() {
	...
	assert_eq!(
		s(unsafe { oaknode_multicam_input_sequence() }),
		"sequence_in"
	);
	assert_eq!(
		s(unsafe { oaknode_multicam_input_sequence_type() }),
		"sequence_type_in"
	);
	...
	let mut sources = oaknode::input::Input::new(
		"sources_in",
		oaknode::value::ValueType::None,
		oaknode::value::NodeValue::None,
	);
	...
	let mut count = -1;
	assert_eq!(
		unsafe { oaknode_multicam_get_source_count(dup(&node), &mut count) },
		OAKNODE_OK
	);
	assert_eq!(count, 4);
	let mut src = -1;
	assert_eq!(
		unsafe { oaknode_multicam_get_current_source(dup(&node), &mut src) },
		OAKNODE_OK
	);
	assert_eq!(src, 2);
...
fn bridge_handle_accessors() {
	...
	assert_eq!(
		unsafe { oaknode_node_get_markers(dup(&math), hp) },
		OAKNODE_OK
	);
	assert!(h.ctx.is_null(), "no markers by default");
	assert_eq!(
		unsafe { oaknode_node_get_work_area(dup(&math), hp) },
		OAKNODE_OK
	);
	assert!(h.ctx.is_null(), "no work area by default");
	assert_eq!(
		unsafe { oaknode_node_get_video_frame_cache(dup(&math), hp) },
		OAKNODE_OK
	);
	...
	let mut out = CHandle::null();
	assert_eq!(
		unsafe { oaknode_node_find_input_footage(dup(&math), &mut out) },
		OAKNODE_OK
	);
	...
	assert_eq!(
		unsafe { oaknode_project_add_node(dup(&p), dup(&footage)) },
		OAKNODE_OK
	);
	assert_eq!(
	...
	let mut seq = unsafe { oaknode_sequence_create() };
	assert_eq!(
		unsafe { oaknode_node_get_markers(dup(&seq), hp) },
		OAKNODE_OK
	);
...
fn viewer_stream_params() {
	...
	let mut vp =
		oaknode::bridge::common::videoparams_init_basic(640, 480, 4, 4, 1, 1, 0, 1).unwrap();
	assert_eq!(
	...
	assert_eq!(
		unsafe {
			oaknode_viewer_set_video_params(dup(&seq), &vp as *const _ as *const std::ffi::c_void)
		},
		OAKNODE_OK
	);
	...
	assert_eq!(
		unsafe {
			oaknode_sequence_get_video_params(
				dup(&seq),
				0,
				&mut vp2 as *mut CHandle as *mut std::ffi::c_void,
			)
		},
		OAKNODE_OK
	);
	...
	assert_eq!(
		oaknode::bridge::common::videoparams_get_width(dup(&vp2)).unwrap(),
		640
	);
	assert_eq!(
		oaknode::bridge::common::videoparams_get_height(dup(&vp2)).unwrap(),
		480
	);
	assert_eq!(
	...
	assert_eq!(
		unsafe { oaknode_sequence_get_audio_params(dup(&seq), 0, &mut ap2) },
		OAKNODE_OK
	);
	assert!(!ap2.is_null());
	assert_eq!(
		oaknode::bridge::core::audioparams_sample_rate(ap2).unwrap(),
		48000
	);
	assert_eq!(
		oaknode::bridge::core::audioparams_channel_layout(ap2).unwrap(),
		0x3
	);
	...
	assert_eq!(
		unsafe {
			oaknode_viewer_set_video_params(
				dup(&seq),
				&empty as *const _ as *const std::ffi::c_void,
			)
		},
		OAKNODE_E_INVALID
	);
...
fn stream_params_roundtrip() {
	...
	let mut vc = 0;
	assert_eq!(
		unsafe { oaknode_sequence_get_video_stream_count(dup(&seq), &mut vc) },
		OAKNODE_OK
	);
	...
	assert_eq!(
		unsafe {
			oaknode_sequence_get_video_params(
				dup(&seq),
				0,
				&mut vp as *mut CHandle as *mut std::ffi::c_void,
			)
		},
		OAKNODE_OK
	);
	...
	assert_eq!(
		oaknode::bridge::common::videoparams_get_width(dup(&vp)).unwrap(),
		1920
	);
	// set_video_params(0) round-trips the same handle.
	assert_eq!(
		unsafe { oaknode_sequence_set_video_params(dup(&seq), 0, dup(&vp)) },
		OAKNODE_OK
	);
	...
	assert_eq!(
		unsafe { oaknode_sequence_set_video_params(dup(&seq), 7, dup(&vp)) },
		OAKNODE_E_NOT_FOUND
	);
	assert_eq!(
		unsafe {
	...
	assert_eq!(
		unsafe { oaknode_sequence_set_video_params(dup(&seq), 0, CHandle::null()) },
		OAKNODE_E_INVALID
	);
	oaknode::bridge::common::videoparams_free(&mut vp);
	...
	let mut ac = 0;
	assert_eq!(
		unsafe { oaknode_sequence_get_audio_stream_count(dup(&seq), &mut ac) },
		OAKNODE_OK
	);
	assert_eq!(ac, 1, "default audio stream");
	let mut ap: *mut std::ffi::c_void = std::ptr::null_mut();
	assert_eq!(
		unsafe { oaknode_sequence_get_audio_params(dup(&seq), 0, &mut ap) },
		OAKNODE_OK
	);
	assert!(!ap.is_null());
	assert_eq!(
		oaknode::bridge::core::audioparams_sample_rate(ap).unwrap(),
		48000
	);
	assert_eq!(
		unsafe { oaknode_sequence_set_audio_params(dup(&seq), 0, ap) },
		OAKNODE_OK
	);
	assert_eq!(
		unsafe { oaknode_sequence_get_audio_params(dup(&seq), 9, &mut ap) },
		OAKNODE_E_NOT_FOUND
	);
	assert_eq!(
		unsafe { oaknode_sequence_set_audio_params(dup(&seq), 9, ap) },
		OAKNODE_E_NOT_FOUND
	);
	assert_eq!(
		unsafe { oaknode_sequence_get_audio_params(dup(&seq), -1, &mut ap) },
		OAKNODE_E_INVALID
	);
	assert_eq!(
		unsafe { oaknode_sequence_set_audio_params(dup(&seq), 0, std::ptr::null()) },
		OAKNODE_E_INVALID
	);
	oaknode::bridge::core::audioparams_free(ap);
	...
	assert_eq!(
		unsafe {
			oaknode_footage_get_video_params(
				dup(&footage),
				0,
				&mut fvp as *mut CHandle as *mut std::ffi::c_void,
			)
		},
		OAKNODE_E_NOT_FOUND
	);
	assert_eq!(
		unsafe {
			oaknode_footage_set_video_params(
				dup(&footage),
				0,
				&fvp as *const _ as *const std::ffi::c_void,
			)
		},
		OAKNODE_E_INVALID,
...
fn colortransform_compliance() {
	...
	let mut t =
		oaknode::bridge::common::colortransform_init_display("sRGB", "Standard", "").unwrap();
	let mut out = CHandle::null();
	assert_eq!(
		unsafe {
			oaknode_colormanager_get_compliant_color_transform(dup(&cm), dup(&t), 0, &mut out)
		},
		OAKNODE_E_STATE
	);
	...
	assert_eq!(
		unsafe {
			oaknode_colormanager_get_compliant_color_transform(dup(&cm), dup(&t2), 0, &mut out)
		},
		OAKNODE_OK
	);
	assert!(!out.ctx.is_null());
	...
	assert_eq!(
		unsafe {
			oaknode_colormanager_get_compliant_color_transform(
				dup(&cm),
				dup(&t),
				0,
				std::ptr::null_mut(),
