...
fn assert_round_trip(name: &str) {
	...
	let doc = oak_otio::from_json_string(&text).unwrap_or_else(|e| panic!("parse {name}: {e}"));
	let out = doc
		.to_json_string()
...
fn golden_timeline_parses_as_timeline_root() {
	let doc = oak_otio::from_json_string(&read_golden("golden_timeline.json")).unwrap();
...
fn golden_collection_parses_as_collection_root() {
	let doc = oak_otio::from_json_string(&read_golden("golden_collection.json")).unwrap();
