...
fn block_trim_trim_in_creates_adjacent() {
	...
	unsafe {
		oaknode_track_prepend_block(t.clone(), prev.clone());
	}
...
fn track_slide_creates_in_and_out_adjacents() {
	...
	unsafe {
		oaknode_track_prepend_block(t.clone(), tail.clone());
	}
	unsafe {
		oaknode_track_prepend_block(t.clone(), b.clone());
	}
...
fn track_slide_removes_in_adjacent() {
	...
	unsafe {
		oaknode_track_prepend_block(t.clone(), g_out.clone());
	}
	unsafe {
		oaknode_track_prepend_block(t.clone(), b.clone());
	}
	unsafe {
		oaknode_track_prepend_block(t.clone(), g_in.clone());
	}
...
fn track_place_appends_gap_past_end() {
	...
	unsafe {
		oaknode_track_prepend_block(t.clone(), existing.clone());
	}
