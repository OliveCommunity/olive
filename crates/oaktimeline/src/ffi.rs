...
use crate::handle::{
	get, get_mut, guard, guard_handle, guard_i32, guard_void, make_owned, CHandle,
};
...
pub mod marker {
	...
	pub unsafe extern "C" fn oaktimeline_marker_add(
		...
		guard(|| {
			...
			let l = unsafe { get_mut::<marker_mod::TimelineMarkerList>(&list) }
				.ok_or(Error::Invalid)?;
	...
	pub unsafe extern "C" fn oaktimeline_marker_count(list: H, out_count: *mut c_int) -> c_int {
		guard(|| {
			...
			let l =
				unsafe { get::<marker_mod::TimelineMarkerList>(&list) }.ok_or(Error::Invalid)?;
			// SAFETY: `out_count` is a valid out pointer.
			unsafe {
				*out_count = l.size() as c_int;
			}
			Ok(())
		})
	}
	...
	pub unsafe extern "C" fn oaktimeline_marker_at(
		...
		guard_i32(|| {
			...
			if !in_num.is_null() {
				...
				unsafe {
					*in_num = t.in_().numerator() as c_int;
				}
			}
			if !in_den.is_null() {
				// SAFETY: `in_den` is a valid out pointer.
				unsafe {
					*in_den = t.in_().denominator() as c_int;
				}
			}
			if !out_num.is_null() {
				// SAFETY: `out_num` is a valid out pointer.
				unsafe {
					*out_num = t.out().numerator() as c_int;
				}
			}
			if !out_den.is_null() {
				// SAFETY: `out_den` is a valid out pointer.
				unsafe {
					*out_den = t.out().denominator() as c_int;
				}
			}
			if !color.is_null() {
	...
	pub unsafe extern "C" fn oaktimeline_marker_remove_at_command(list: H, index: c_int) -> H {
		guard_handle(|| -> Result<CHandle> {
			let in_bounds = {
				...
				// SAFETY: the boxed value is a `TimelineMarkerList`.
				let l = unsafe { get::<marker_mod::TimelineMarkerList>(&list) }
					.ok_or(Error::Invalid)?;
	...
	pub unsafe extern "C" fn oaktimeline_marker_set_props_command(
		...
		guard_handle(|| -> Result<CHandle> {
			...
			if color >= 0 && !name.is_null() {
				...
				let color_child =
					marker_mod::MarkerChangeColorCommand::new(list.clone(), index as usize, color)
						.to_command();
				// SAFETY: `multi` is a valid multi command handle.
				let _ =
					unsafe { oundo::oakundo_command_multi_add_child(multi.clone(), color_child) };
				let name_child =
					marker_mod::MarkerChangeNameCommand::new(list, index as usize, &name_str)
						.to_command();
				// SAFETY: `multi` is a valid multi command handle.
				let _ =
					unsafe { oundo::oakundo_command_multi_add_child(multi.clone(), name_child) };
				return Ok(multi);
			}
			if color >= 0 {
				return Ok(
					marker_mod::MarkerChangeColorCommand::new(list, index as usize, color)
						.to_command(),
				);
			}
			Ok(
	...
	pub unsafe extern "C" fn oaktimeline_marker_list_load(list: H, reader: H) -> c_int {
		guard(|| {
			...
			// the call.
			let default_color =
				unsafe { xml::oakcommon_config_get_int(std::ptr::null(), group.as_ptr(), 0) };
			loop {
				...
				if element == "marker" {
					...
					let _ = unsafe {
						xml::oakcommon_xml_reader_attribute_count(reader.clone(), &mut count)
					};
					for i in 0..count {
...
pub mod workarea {
	...
	pub unsafe extern "C" fn oaktimeline_workarea_set_enabled(w: H, enabled: c_int) -> c_int {
		guard(|| {
			...
			// exclusive access for the duration of the call.
			let wa =
				unsafe { get_mut::<workarea_mod::TimelineWorkArea>(&w) }.ok_or(Error::Invalid)?;
	...
	pub unsafe extern "C" fn oaktimeline_workarea_get(
		...
		guard(|| {
			...
			if !in_num.is_null() {
				// SAFETY: `in_num` is a valid out pointer.
				unsafe {
					*in_num = wa.in_().numerator() as c_int;
				}
			}
			if !in_den.is_null() {
				// SAFETY: `in_den` is a valid out pointer.
				unsafe {
					*in_den = wa.in_().denominator() as c_int;
				}
			}
			if !out_num.is_null() {
				// SAFETY: `out_num` is a valid out pointer.
				unsafe {
					*out_num = wa.out().numerator() as c_int;
				}
			}
			if !out_den.is_null() {
				// SAFETY: `out_den` is a valid out pointer.
				unsafe {
					*out_den = wa.out().denominator() as c_int;
				}
			}
			if !enabled.is_null() {
				// SAFETY: `enabled` is a valid out pointer.
				unsafe {
					*enabled = wa.enabled() as c_int;
				}
			}
			Ok(())
		})
	...
	pub unsafe extern "C" fn oaktimeline_workarea_set_range_command(
		...
		guard_handle(|| -> Result<CHandle> {
			...
			Ok(
				workarea_mod::WorkareaSetRangeCommand::new_with_old(w, range, old_range)
					.to_command(),
			)
		})
	}
	...
	pub unsafe extern "C" fn oaktimeline_workarea_save(w: H, writer: H) -> c_int {
		guard(|| {
			...
			// SAFETY: `writer` and both C strings are valid for the call.
			unsafe {
				xml::oakcommon_xml_writer_write_text_element(
					writer.clone(),
					enabled_key.as_ptr(),
					enabled_val.as_ptr(),
				)
...
/// place_block_command / replace_block_with_gap_command / move_block_command /
/// trim_command / split_command / split_preserving_links_command /
...
pub mod edit {
	...
	/// `oaktimeline_move_block_command`: owned `TrackMoveBlockCommand` — gap
	/// the block's old spot and place it at `in_` on `track_index` as one
	/// undoable entry (the capi's `oakengine_sequence_move_clip` assembly).
	#[no_mangle]
	pub unsafe extern "C" fn oaktimeline_move_block_command(
		list: H,
		track_index: c_int,
		block: H,
		in_num: i64,
		...
		guard_handle(|| -> Result<CHandle> {
			...
			Ok(
				TrackMoveBlockCommand::new(list, track_index, block, Rational::new(in_num, in_den))
					.to_command(),
			)
		})
	}
	...
	pub unsafe extern "C" fn oaktimeline_split_preserving_links_command(
		...
		guard_handle(|| -> Result<CHandle> {
			...
			let block_vec: Vec<CHandle> =
				unsafe { std::slice::from_raw_parts(blocks, count as usize) }.to_vec();
	...
	pub unsafe extern "C" fn oaktimeline_slide_command(
		...
		guard_handle(|| -> Result<CHandle> {
			...
			// non-NULL and positive above), readable for the duration of the call.
			let block_vec: Vec<CHandle> =
				unsafe { std::slice::from_raw_parts(blocks, block_count as usize) }.to_vec();
...
fn xml_read_next(reader: &H) -> String {
	...
	let r =
		unsafe { xml::oakcommon_xml_reader_read_next_start_element(reader.clone(), &mut found) };
	if r <= 0 || found == 0 {
		return String::new();
	}
	...
	let n = unsafe {
		xml::oakcommon_xml_reader_name(reader.clone(), buf.as_mut_ptr(), buf.len() as c_int)
	};
	if n <= 0 {
		return String::new();
	}
...
fn xml_element_text(reader: &H) -> String {
	...
	// SAFETY: `buf` is a valid writable buffer; `reader` is a valid handle.
	let r = unsafe {
		xml::oakcommon_xml_reader_read_element_text(
			reader.clone(),
			buf.as_mut_ptr(),
			buf.len() as c_int,
		)
...
fn xml_attribute(reader: &H, index: c_int) -> (String, String) {
	...
	let _ = unsafe {
		xml::oakcommon_xml_reader_attribute_name(
			reader.clone(),
			index,
			name_buf.as_mut_ptr(),
	...
	let _ = unsafe {
		xml::oakcommon_xml_reader_attribute_value(
			reader.clone(),
			index,
