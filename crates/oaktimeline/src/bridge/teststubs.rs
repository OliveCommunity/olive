...
use crate::handle::{get, get_mut, make_owned, CHandle, RefBox};
...
pub fn oakundo_stack_push(stack: CHandle, _command: CHandle, _text: *const c_char) -> c_int {
...
/// `oaknode_block_set_in`: set the in point, keeping the length (the out
/// point follows; media-in unchanged).
pub fn oaknode_block_set_in(block: CHandle, numerator: c_int, denominator: c_int) -> c_int {
	// SAFETY: handle boxes a MockNode.
	let Some(b) = (unsafe { get_mut::<MockNode>(&block) }) else {
		return -1;
	};
	b.in_ = (numerator, denominator);
	b.out = add_pair(b.in_, b.length);
	0
}
...
pub fn oaknode_track_get_nearest_block_before_or_at(
	...
	for &p in &t.blocks {
		...
		if pair_cmp(b.in_, time) != std::cmp::Ordering::Greater {
			if best.is_null()
				|| pair_cmp(b.in_, unsafe { &*best }.in_) == std::cmp::Ordering::Greater
			{
				best = p;
			}
		}
...
pub fn oaknode_sequence_get_track_list(sequence: CHandle, kind: c_int, out: *mut CHandle) -> c_int {
	// SAFETY: handle boxes a MockNode.
