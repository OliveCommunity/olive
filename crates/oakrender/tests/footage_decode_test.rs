...
fn footage_decode_renders_known_content() {
	...
	oakcodec::testmedia::write_test_clip(&path, 64, 64, 10, 10).expect("test clip generation");
	...
	let proxy = oakrender::eval::render_footage_frame(
		&path.to_string_lossy(),
		0,
		Rational::new(0, 1),
		(32, 32),
...
fn footage_decode_fails_explainably_for_missing_file() {
	let err = oakrender::eval::render_footage_frame(
