...
//!
//! Serializes a borrowed `oak_node::project::Project` to an `.oakproj` file
//! via the direct Rust serializer (`oak_node::serializer::save` —
...
impl TaskBehavior for ProjectSaveTask {
	...
	/// (`oak_node::serializer::save`) to the resolved filename.
	fn run(&mut self, task: &mut Task) -> Result<()> {
		...
		let xml = match oak_node::serializer::save(
