...
ProjectSerializer210528::load(Project *project, XmlStreamReader *reader,
		...
		if (reader->name() == "input") {
			...
			while (xml_read_next_start_element(reader)) {
				if (reader->name() == "connection") {
					...
					std::string output_param_id;

					while (xml_read_next_start_element(reader)) {
						if (reader->name() == "node") {
							output_node_id = reader->read_element_text();
						} else if (reader->name() == "output") {
							output_param_id = reader->read_element_text();
						} else {
							reader->skip_current_element();
						}
					}
...
void ProjectSerializer210528::load_color_manager(XmlStreamReader *reader,
...
void ProjectSerializer210528::load_project_settings(XmlStreamReader *reader,
												  Project *project) const
{
	while (xml_read_next_start_element(reader)) {
		if (reader->name() == "input") {
...
void ProjectSerializer210528::load_input(Node *node, XmlStreamReader *reader,
...
void ProjectSerializer210528::load_immediate(XmlStreamReader *reader,
											Node *node,
...
bool ProjectSerializer210528::load_position(XmlStreamReader *reader,
										   uintptr_t *node_ptr,
...
void ProjectSerializer210528::load_node_custom(XmlStreamReader *reader,
											 Node *node,
											 XMLNodeData &xml_node_data) const
{
	// Viewer-based nodes
...
void ProjectSerializer210528::load_timeline_points(XmlStreamReader *reader,
