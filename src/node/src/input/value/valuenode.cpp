...
const std::string ValueNode::k_type_input = "type_in";
const std::string ValueNode::k_value_input = "value_in";
const std::vector<NodeValue::Type> ValueNode::k_supported_types = {
...
ValueNode::ValueNode()
{
	...
	add_input(k_value_input, k_supported_types.front(), Variant(),
...
void ValueNode::retranslate()
{
	...
	set_input_name(k_value_input, "Value");

	StringList type_names;
	type_names.reserve(k_supported_types.size());
	for (NodeValue::Type type : k_supported_types) {
		type_names.push_back(NodeValue::get_pretty_data_type_name(type));
	}
					  ...
					  NodeValueTable *table) const
{
	...
	table->push(value.at(k_value_input));
}

void ValueNode::InputValueChangedEvent(const std::string &input, int element)
{
	if (input == k_type_input) {
		set_input_data_type(
			k_value_input,
			k_supported_types.at(get_standard_value(k_type_input).to_int()));
	}
