...
public:
	...
	NodeValue generate_row_value(const Node *node, const std::string &input,
							   NodeValueTable *table, const TimeRange &time);
	NodeValue generate_row_value_element(const Node *node, const std::string &input,
	...
	int generate_row_value_element_index(const Node *node, const std::string &input,
									 int element, const NodeValueTable *table);

	void transform(Matrix4x4 *transform, const Node *start, const Node *end,
				   const TimeRange &range);
...
protected:
	NodeValueTable process_input(const Node *node, const std::string &input,
								const TimeRange &range);
	...
	Vector2D generate_resolution() const;
...
private:
	...
	/**
	 * @brief Ordering for TimeRange keys in the value cache (QHash
	 * replacement; std::map needs a strict weak ordering).
	 */
	...
	std::map<const Node *, std::map<TimeRange, NodeValueTable, TimeRangeLess>>
		value_cache_;
	std::map<Texture *, TexturePtr> resolved_texture_cache_;
};
