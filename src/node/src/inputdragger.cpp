			...
			if (create_key_on_all_tracks) {
				...
				for (int i = 0; i < nb_tracks; i++) {
					if (i != input.track()) {
						...
						created_keys_.push_back(new NodeKeyframe(
							time,
...
void NodeInputDragger::drag(Variant value)
{
	assert(is_started());

	Node *node = input_.input().node();
	const std::string &input = input_.input().input();

	if (node->has_input_property(input, "min")) {
		// Assumes the value is a double of some kind
		double min =
			node->get_input_property(input, "min").to_double();
	...
	if (node->has_input_property(input, "max")) {
		double max =
			node->get_input_property(input, "max").to_double();
		double v = value.to_double();
		if (v > max) {
			value = max;
		}
...
void NodeInputDragger::end(MultiUndoCommand *command)
{
	...
	if (input_.input().node()->is_input_keyframing(input_.input())) {
		for (int i = 0; i < int(created_keys_.size()); i++) {
