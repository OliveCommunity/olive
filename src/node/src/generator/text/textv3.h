...
#include "generator/shape/shapenodebase.h"
...
public:
	...
	VerticalAlignment get_vertical_alignment() const
	{
		return static_cast<VerticalAlignment>(
			get_standard_value(k_vertical_alignment_input).to_int());
	}

	// Alignment values travel as ints now (formerly Qt::Alignment), matching
	// TextGizmo::VerticalAlignment (0 = top, 1 = bottom, 2 = vcenter)
	static int get_qt_alignment_from_ours(VerticalAlignment v);
	static VerticalAlignment get_our_alignment_from_qts(int v);

	static const std::string k_text_input;
	static const std::string k_vertical_alignment_input;
	static const std::string k_use_args_input;
	static const std::string k_args_input;
...
private:
	...
	// Formerly private slots connected to TextGizmo signals; the gizmo wave /
	// facade wires these up to the gizmo's notifications directly
	void gizmo_activated();
	void gizmo_deactivated();
	void set_vertical_alignment_undoable(int a);
};
