...
const std::string GeneratorWithMerge::k_base_input = "base_in";
...
void GeneratorWithMerge::retranslate()
{
	...
	set_input_name(k_base_input, "Base");
}
...
{
	if (TexturePtr base = value.at(k_base_input).to_texture()) {
		// Push as merge node
		ShaderJob merge;

		merge.set_shader_id("mrg");
		merge.insert(MergeNode::k_base_in, value.at(k_base_input));
