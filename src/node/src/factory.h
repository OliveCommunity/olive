...
public:
	...
	static std::string get_name_from_id(const std::string &id);

	static Node *create_from_id(const std::string &id);
	 ...
	 * UI-independent.
	 */
	static const std::vector<Node *> &get_library()
	{
		return library;
	}

private:
	static std::vector<Node *> library;
};
