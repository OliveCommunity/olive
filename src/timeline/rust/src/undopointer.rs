...
use crate::bridge::node::{
  oaknode_block_gap_create, oaknode_block_get_kind, oaknode_track_get_block_at,
  oaknode_track_get_block_count, oaknode_track_insert_block_after, oaknode_track_prepend_block,
  oaknode_track_ripple_remove_block, oaknode_tracklist_get_track_at, oaknode_tracklist_get_track_count,
};
use crate::bridge::undo::{oakundo_command_redo_now, oakundo_command_undo_now};
use crate::handle::CHandle;
use crate::undocommon::{
  block_can_be_removed, box_command, create_and_run_block_remove_command, create_block_remove_command,
  Command,
};
use crate::util::{
  block_add_to_graph, block_length, block_next, block_previous, block_remove_from_graph,
  block_set_length_and_media_in, block_set_length_and_media_out, track_length,
};

use super::undogeneral::TimelineAddTrackCommand;
...
fn track_append_block(track: CHandle, block: CHandle) {
  let mut count = 0;
...
fn track_insert_block_before(track: CHandle, block: CHandle, next: CHandle) {
  ...
  pub fn prepare(&mut self) {
    ...
    if self.needs_adjacent_ {
      ...
      if !self.adjacent_.is_null() {
        ...
        let _ = unsafe { oaknode_block_get_kind(self.adjacent_.clone(), &mut adjacent_kind) };
      }
      ...
      if self.we_created_adjacent_ {
        ...
        self.we_removed_adjacent_ = (block_length(self.adjacent_.clone()) + self.trim_diff_).is_null();
      }
    }
  }
  ...
  pub fn redo(&mut self) {
    ...
    if self.needs_adjacent_ {
      if self.we_created_adjacent_ {
        ...
        if self.mode == MovementMode::TrimIn {
          track_insert_block_before(self.track.clone(), self.adjacent_.clone(), self.block.clone());
        } else {
          // SAFETY: `track`/`adjacent_`/`block` are valid handles.
          let _ = unsafe {
            oaknode_track_insert_block_after(self.track.clone(), self.adjacent_.clone(), self.block.clone())
          };
        }
        self.adjacent_orphaned_ = false;
      } else if self.we_removed_adjacent_ {
        // SAFETY: valid handles.
        let _ = unsafe { oaknode_track_ripple_remove_block(self.track.clone(), self.adjacent_.clone()) };
        ...
        if self.remove_block_from_graph_ && block_can_be_removed(self.adjacent_.clone()) {
          if self.deleted_adjacent_command_.is_null() {
            self.deleted_adjacent_command_ = create_and_run_block_remove_command(self.adjacent_.clone());
          } else {
            // SAFETY: `deleted_adjacent_command_` is a valid command handle.
            let _ = unsafe { oakundo_command_redo_now(self.deleted_adjacent_command_.clone()) };
          }
        }
      } else {
      ...
      if self.we_created_adjacent_ {
        ...
        if self.we_removed_adjacent_ {
          if !self.deleted_adjacent_command_.is_null() {
            ...
            // SAFETY: `deleted_adjacent_command_` is a valid command handle.
            let _ = unsafe { oakundo_command_undo_now(self.deleted_adjacent_command_.clone()) };
          }
...
impl TrackSlideCommand {
  ...
  pub fn prepare(&mut self) {
    ...
    if self.out_adjacent.is_null() && !block_next(self.blocks[self.blocks.len() - 1].clone()).is_null() {
      // SAFETY: `oaknode_block_gap_create` returns a fresh owned handle.
      self.out_adjacent = unsafe { oaknode_block_gap_create() };
      block_set_length_and_media_out(self.out_adjacent.clone(), Rational::new(0, 1) - self.movement);
  ...
  pub fn redo(&mut self) {
    ...
    if self.we_created_in_adjacent_ {
      ...
      track_insert_block_before(self.track.clone(), self.in_adjacent.clone(), self.blocks[0].clone());
      ...
      // SAFETY: valid handles.
      let _ = unsafe { oaknode_track_ripple_remove_block(self.track.clone(), self.in_adjacent.clone()) };

      if block_can_be_removed(self.in_adjacent.clone()) {
        if self.in_adjacent_remove_command_.is_null() {
          self.in_adjacent_remove_command_ = create_block_remove_command(self.in_adjacent.clone());
        }

        // SAFETY: `in_adjacent_remove_command_` is a valid command handle.
        let _ = unsafe { oakundo_command_redo_now(self.in_adjacent_remove_command_.clone()) };
      }
    ...
    if !self.out_adjacent.is_null() {
      if self.we_created_out_adjacent_ {
        ...
        // SAFETY: valid handles.
        let _ = unsafe { oaknode_track_ripple_remove_block(self.track.clone(), self.out_adjacent.clone()) };

        if block_can_be_removed(self.out_adjacent.clone()) {
          if self.out_adjacent_remove_command_.is_null() {
            self.out_adjacent_remove_command_ = create_block_remove_command(self.out_adjacent.clone());
          }

          // SAFETY: `out_adjacent_remove_command_` is a valid command handle.
          let _ = unsafe { oakundo_command_redo_now(self.out_adjacent_remove_command_.clone()) };
        }
  ...
  pub fn undo(&mut self) {
    if self.we_created_in_adjacent_ {
      ...
      if !self.in_adjacent_remove_command_.is_null() {
        ...
        // SAFETY: `in_adjacent_remove_command_` is a valid command handle.
        let _ = unsafe { oakundo_command_undo_now(self.in_adjacent_remove_command_.clone()) };
      }
    ...
    if !self.out_adjacent.is_null() {
      if self.we_created_out_adjacent_ {
        ...
        if !self.out_adjacent_remove_command_.is_null() {
          // We removed this, so we can restore it now
          // SAFETY: `out_adjacent_remove_command_` is a valid command handle.
          let _ = unsafe { oakundo_command_undo_now(self.out_adjacent_remove_command_.clone()) };
        }
...
impl TrackPlaceBlockCommand {
  ...
  pub fn redo(&mut self) {
    ...
    let _ = unsafe { oaknode_tracklist_get_track_count(self.timeline.clone(), &mut track_count) };
    ...
    if self.track_index >= track_count {
      if self.add_track_commands_.is_empty() {
        ...
        for _ in 0..(self.track_index - track_count + 1) {
          self.add_track_commands_.push(TimelineAddTrackCommand::new(self.timeline.clone()));
        }
      }
    ...
    let _ = unsafe { oaknode_tracklist_get_track_at(self.timeline.clone(), self.track_index, &mut track) };
    ...
    if append {
      if in_ > track_length(track.clone()) {
        ...
        if self.gap_.is_null() {
          ...
          block_set_length_and_media_out(self.gap_.clone(), in_ - track_length(track.clone()));
        }
      ...
      if self.ripple_remove_command_.is_none() {
        ...
        let mut cmd = TrackRippleRemoveAreaCommand::new(track.clone(), TimeRange::new(in_, in_ + insert_length));
      ...
      let index = self.ripple_remove_command_.as_ref().unwrap().get_insertion_index();
  ...
  pub fn undo(&mut self) {
    ...
    let _ = unsafe { oaknode_tracklist_get_track_at(self.timeline.clone(), self.track_index, &mut t) };
