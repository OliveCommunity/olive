...
// Interprets ABI Variant payloads without copying; this ABI version assumes
// the host and backend are built with the same C++ ABI.
...
{
	// The QObject-style parent argument is retained for ABI parity and ignored.
	(void) parent;
	return new BackendVulkanRenderer();
}
...
// Vulkan does not use an OpenGL context; the argument is accepted for ABI parity.
OAK_RENDER_BACKEND_EXPORT void
...
// Creates a Vulkan texture and writes its Variant handle to out_variant.
...
// Destroys a Vulkan texture represented by a Variant handle.
OAK_RENDER_BACKEND_EXPORT void
...
// Compiles a Vulkan shader and returns its Variant handle.
OAK_RENDER_BACKEND_EXPORT void
...
// Destroys a Vulkan shader represented by a Variant handle.
OAK_RENDER_BACKEND_EXPORT void
