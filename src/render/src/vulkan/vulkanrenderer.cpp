...
struct VulkanRenderer::VulkanShader {
	...
	std::map<VkFormat, VkPipeline> pipeline_cache;
	...
	std::map<std::string, int> sampler_bindings;
};
...
// std::map equivalents of the QHash helpers used by the original code.
template <typename K, typename V>
static V *map_lookup(const std::map<K, V *> &m, const K &key)
{
	auto it = m.find(key);
...
template <typename K, typename V>
static V *map_take(std::map<K, V *> &m, const K &key)
{
	auto it = m.find(key);
	if (it == m.end()) {
		return nullptr;
...
static int map_value_or(const std::map<std::string, int> &m,
...
// std::string equivalents of the QString helpers used by the original code.
static std::string string_trimmed(const std::string &s)
{
	const char *ws = " \t\r\n\f\v";
...
static void string_replace_all(std::string *s, const std::string &from,
							   const std::string &to)
{
	if (from.empty()) {
		return;
...
// One GLSL uniform declaration match, mirroring the captures of the original
// QRegularExpression scan: byte offset/length of the whole declaration plus
// the type and name capture groups.
struct UniformDeclMatch {
	size_t offset;
	size_t length;
	std::string type;
	std::string name;
...
// Scans for `uniform <type> <name>;` declarations anchored at line starts
// (std::regex has no multiline mode, so lines are scanned individually, which
// reproduces QRegularExpression::MultilineOption `^` semantics).
static std::vector<UniformDeclMatch> find_uniform_decls(const std::string &glsl)
{
	static const std::regex re(R"(\s*uniform\s+(\w+)\s+(\w+)\s*;)");
	std::vector<UniformDeclMatch> matches;
	size_t line_start = 0;
	while (line_start <= glsl.size()) {
		size_t line_end = glsl.find('\n', line_start);
		size_t line_len = (line_end == std::string::npos) ?
							  glsl.size() - line_start :
							  line_end - line_start;
		std::string line = glsl.substr(line_start, line_len);
		std::smatch m;
		if (std::regex_search(line, m, re) && m.position(0) == 0) {
			matches.push_back({ line_start, static_cast<size_t>(m.length(0)),
								m[1].str(), m[2].str() });
		}
...
static const float k_blit_vertices[] = {
...
bool VulkanRenderer::init()
{
	...
	return create_instance() && create_device() && create_command_pool() &&
		   create_descriptor_pool();
}

// Creates reusable draw resources after Init() has a valid logical device.
void VulkanRenderer::post_init()
{
...
void VulkanRenderer::post_destroy()
{
}
...
void VulkanRenderer::destroy_internal()
{
...

	{
		...
		for (auto it = shaders_.begin(); it != shaders_.end(); ++it) {
			VulkanShader *sh = it->second;
			for (auto pit = sh->pipeline_cache.begin();
				 pit != sh->pipeline_cache.end(); ++pit) {
				if (pit->second != VK_NULL_HANDLE) {
					vkDestroyPipeline(device_, pit->second, nullptr);
				}
			}
	...
	for (auto it = render_pass_cache_.begin(); it != render_pass_cache_.end();
		...
		if (it->second != VK_NULL_HANDLE) {
			vkDestroyRenderPass(device_, it->second, nullptr);
		}
	}
...
bool VulkanRenderer::create_instance()
{
	...
	const bool enable_validation =
		std::getenv("OAK_VULKAN_VALIDATION") != nullptr;
	...
	fprintf(stderr, "Vulkan instance created successfully\n");
	return true;
}
	...
	const VkDebugUtilsMessengerCallbackDataEXT *p_callback_data, void *p_user_data)
{
	...
	if (message_severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT) {
		...
		fprintf(stderr, "Vulkan validation warning: %s\n", p_callback_data->pMessage);
	}

	return VK_FALSE;
}

bool VulkanRenderer::create_debug_messenger()
{
	...
	if (result != VK_SUCCESS) {
		fprintf(stderr, "Failed to create Vulkan debug messenger: %d\n", result);
		return false;
	}
	return true;
}
...
bool VulkanRenderer::create_device()
{
	...
	if (result != VK_SUCCESS || physical_device_count_ == 0) {
		fprintf(stderr, "No Vulkan-capable physical devices found\n");
		return false;
	}

	std::vector<VkPhysicalDevice> devices(physical_device_count_);
	result = vkEnumeratePhysicalDevices(instance_, &physical_device_count_,
										devices.data());
	if (result != VK_SUCCESS) {
		fprintf(stderr, "Failed to enumerate Vulkan physical devices: %d\n", result);
		return false;
	}
	...
	if (physical_device_ == VK_NULL_HANDLE ||
		...
		fprintf(stderr, "No Vulkan physical device with a graphics queue found\n");
		return false;
	}
	...
	result = vkCreateDevice(physical_device_, &device_create_info, nullptr,
							&device_);
	if (result != VK_SUCCESS) {
		fprintf(stderr, "Failed to create Vulkan logical device: %d\n", result);
		return false;
	}

	fprintf(stderr, "Vulkan device created successfully on %s\n", device_properties_.deviceName);
...
bool VulkanRenderer::create_descriptor_pool()
{
	...
	pool_sizes[0].descriptorCount = k_max_descriptor_sets;
	pool_sizes[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
	pool_sizes[1].descriptorCount = k_max_descriptor_sets * 8;

	VkDescriptorPoolCreateInfo pool_info = {};
	pool_info.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
	pool_info.poolSizeCount = 2;
	pool_info.pPoolSizes = pool_sizes;
	pool_info.maxSets = k_max_descriptor_sets;
	pool_info.flags = VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT;

	VkResult result =
		vkCreateDescriptorPool(device_, &pool_info, nullptr, &descriptor_pool_);
	if (result != VK_SUCCESS) {
		fprintf(stderr, "Failed to create Vulkan descriptor pool: %d\n", result);
		return false;
	}
	return true;
}
...
VkRenderPass VulkanRenderer::get_or_create_render_pass(VkFormat format, bool clear)
{
	const uint64_t key = (static_cast<uint64_t>(format) << 1) |
						(clear ? 1ULL : 0ULL);
	...
	if (result != VK_SUCCESS) {
		fprintf(stderr, "Failed to create Vulkan render pass: %d\n", result);
...
{
	VkDeviceSize buffer_size = sizeof(k_blit_vertices);
	...
	memcpy(data, k_blit_vertices, (size_t)buffer_size);
...
bool VulkanRenderer::create_linear_sampler()
{
	...
	if (result != VK_SUCCESS) {
		fprintf(stderr, "Failed to create Vulkan linear sampler: %d\n", result);
		return false;
	}
	return true;
}
...
bool VulkanRenderer::create_nearest_sampler()
{
	VkSamplerCreateInfo sampler_info = {};
	sampler_info.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
	...
	if (result != VK_SUCCESS) {
		fprintf(stderr, "Failed to create Vulkan nearest sampler: %d\n", result);
		return false;
	}
	return true;
}
...
VkSampler VulkanRenderer::get_sampler(Texture::Interpolation interpolation) const
{
										 ...
										 VkDeviceMemory *out_memory)
{
	...
	if (result != VK_SUCCESS) {
		fprintf(stderr,
				"Failed to create Vulkan staging buffer: %d size=%llu\n",
				result, static_cast<unsigned long long>(size));
		return false;
	}
	...
	alloc_info.memoryTypeIndex = find_memory_type(
		mem_req.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
									VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
	if (alloc_info.memoryTypeIndex == UINT32_MAX) {
		fprintf(stderr, "Failed to find host-visible memory type for Vulkan staging buffer\n");
	...
	if (result != VK_SUCCESS) {
		fprintf(stderr,
				"Failed to allocate Vulkan staging buffer memory: %d size=%llu "
				"allocation=%llu\n",
				result, static_cast<unsigned long long>(size),
				static_cast<unsigned long long>(mem_req.size));
		vkDestroyBuffer(device_, buffer, nullptr);
		return false;
	}
	...
	if (result != VK_SUCCESS) {
		fprintf(stderr, "Failed to bind Vulkan staging buffer memory: %d\n", result);
...
void VulkanRenderer::destroy_staging_buffer(VkBuffer buffer,
...
VkCommandBuffer VulkanRenderer::begin_one_time_commands()
{
	if (reusable_command_buffer_ == VK_NULL_HANDLE) {
		...
		if (result != VK_SUCCESS ||
			reusable_command_buffer_ == VK_NULL_HANDLE) {
			fprintf(stderr, "Failed to allocate Vulkan command buffer: %d\n", result);
			return VK_NULL_HANDLE;
		}
	} else {
	...
	if (result != VK_SUCCESS) {
		fprintf(stderr, "Failed to begin Vulkan command buffer: %d\n", result);
		return VK_NULL_HANDLE;
	}
...
void VulkanRenderer::end_one_time_commands(VkCommandBuffer cmd)
{
	if (cmd == VK_NULL_HANDLE) {
		return;
	}
	...
	if (reusable_fence_ == VK_NULL_HANDLE) {
		...
		if (result != VK_SUCCESS) {
			fprintf(stderr, "Failed to create Vulkan fence: %d\n", result);
			return;
		}
	} else {
	...
	constexpr uint64_t k_timeout_ns = 10ULL * 1000ULL * 1000ULL * 1000ULL;
	result = vkWaitForFences(device_, 1, &reusable_fence_, VK_TRUE, k_timeout_ns);
	if (result == VK_TIMEOUT) {
		fprintf(stderr,
				"Vulkan GPU wait timed out; the GPU may be hung\n");
...
void VulkanRenderer::transition_image_layout(VkCommandBuffer cmd, VkImage image,
										   ...
										   VkImageLayout new_layout)
{
	...
	if (!handled) {
		fprintf(stderr, "Unhandled Vulkan layout transition fromto- using conservative ALL_COMMANDS barrier %d %d\n", old_layout, new_layout);
...
void VulkanRenderer::copy_buffer_to_image(VkCommandBuffer cmd, VkBuffer buffer,
...
void VulkanRenderer::copy_image_to_buffer(VkCommandBuffer cmd, VkImage image,
...
VkFormat VulkanRenderer::pixel_format_to_vk_format(PixelFormat format,
...
bool VulkanRenderer::is_color_attachment_supported(VkFormat format) const
{
...
VkFormat VulkanRenderer::pick_renderable_format(PixelFormat format,
											  int channel_count) const
{
	VkFormat candidate = pixel_format_to_vk_format(format, channel_count);
	if (candidate != VK_FORMAT_UNDEFINED &&
		is_color_attachment_supported(candidate)) {
		return candidate;
	}
	...
	if (channel_count == 3) {
		VkFormat rgba = pixel_format_to_vk_format(format, 4);
		if (rgba != VK_FORMAT_UNDEFINED && is_color_attachment_supported(rgba)) {
			return rgba;
		}
	}
...
int VulkanRenderer::get_vk_format_bytes_per_pixel(VkFormat format) const
{
	switch (format) {
...
float VulkanRenderer::get_format_max_alpha(PixelFormat format) const
{
...
void VulkanRenderer::copy_pixels_with_channel_conversion(
	const void *src, void *dst, int width, int height, int depth,
	int src_channels, int dst_channels, PixelFormat format) const
{
	int src_bpc = VideoParams::get_bytes_per_channel(format);
	int dst_bpc = src_bpc;
	float alpha = get_format_max_alpha(format);
...
VkDeviceSize VulkanRenderer::align_size(VkDeviceSize size,
...
Variant VulkanRenderer::create_native_texture(int width, int height, int depth,
											 PixelFormat format,
											 int channel_count,
											 const void *data, int linesize)
{
	...
	VkFormat vk_format = pick_renderable_format(format, channel_count);
	if (vk_format == VK_FORMAT_UNDEFINED) {
		fprintf(stderr, "Unsupported pixel format for Vulkan texture\n");
		return Variant();
	}
	...
	alloc_info.memoryTypeIndex = find_memory_type(
		mem_req.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
	if (alloc_info.memoryTypeIndex == UINT32_MAX) {
		fprintf(stderr, "Failed to find device-local memory type for Vulkan image\n");
		vkDestroyImage(device_, tex->image, nullptr);
		delete tex;
		return Variant();
	}
	...
	if (result != VK_SUCCESS) {
		fprintf(stderr, "Failed to allocate device memory for Vulkan image: %d\n", result);
		vkDestroyImage(device_, tex->image, nullptr);
		delete tex;
		return Variant();
	}
	...
	if (result != VK_SUCCESS) {
		fprintf(stderr, "Failed to bind Vulkan image memory: %d\n", result);
		vkFreeMemory(device_, tex->memory, nullptr);
		vkDestroyImage(device_, tex->image, nullptr);
		delete tex;
		return Variant();
	...
	if (data) {
		...
		int gpu_bytes_per_pixel = get_vk_format_bytes_per_pixel(vk_format);
		...
		VkBuffer staging_buffer;
		VkDeviceMemory staging_memory;
		if (create_staging_buffer(image_size, &staging_buffer, &staging_memory)) {
			void *mapped;
			...
			if (cpu_bytes_per_pixel == gpu_bytes_per_pixel) {
				...
				ByteArray tmp(width * height * depth * cpu_bytes_per_pixel);
				if (linesize == width) {
				...
				int gpu_channels = gpu_bytes_per_pixel /
								   VideoParams::get_bytes_per_channel(format);
				copy_pixels_with_channel_conversion(tmp.data(), mapped, width,
			...
			transition_image_layout(cmd, tex->image, VK_IMAGE_LAYOUT_UNDEFINED,
								  VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
			copy_buffer_to_image(cmd, staging_buffer, tex->image,
			...
			destroy_staging_buffer(staging_buffer, staging_memory);
	...
	return Variant::from_value(tex->id);
}

// Destroys a texture handle and all Vulkan objects owned by that texture.
void VulkanRenderer::destroy_native_texture(Variant texture)
{
	std::lock_guard<std::mutex> lock(mutex_);
	uint64_t id = texture.value<uint64_t>();
	VulkanTexture *tex = map_take(textures_, id);
	if (!tex) {
		return;
	}
...
void VulkanRenderer::upload_to_texture(const Variant &handle,
									 const VideoParams &params,
									 const void *data, int linesize)
{
	std::lock_guard<std::mutex> lock(mutex_);
	uint64_t id = handle.value<uint64_t>();
	VulkanTexture *tex = map_lookup(textures_, id);
	if (!tex || !data) {
		return;
	}
	...
	int cpu_bytes_per_pixel =
		VideoParams::get_bytes_per_pixel(params.format(), params.channel_count());
	int gpu_bytes_per_pixel = get_vk_format_bytes_per_pixel(tex->vk_format);
	if (gpu_bytes_per_pixel == 0) {
		gpu_bytes_per_pixel = cpu_bytes_per_pixel;
	}
	...
	if (cpu_bytes_per_pixel == gpu_bytes_per_pixel) {
		...
		int gpu_channels = gpu_bytes_per_pixel /
						   VideoParams::get_bytes_per_channel(params.format());
		copy_pixels_with_channel_conversion(tmp.data(), mapped, width, height,
	...
	if (tex->current_layout != VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
		transition_image_layout(cmd, tex->image, tex->current_layout,
							  VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
	}
	copy_buffer_to_image(cmd, staging_buffer, tex->image,
					  static_cast<uint32_t>(width),
...
void VulkanRenderer::download_from_texture(const Variant &handle,
										 ...
										 int linesize)
{
	...
	copy_image_to_buffer(cmd, tex->image, staging_buffer,
					  static_cast<uint32_t>(width),
					  static_cast<uint32_t>(height));
	end_one_time_commands(cmd);
	...
	if (cpu_bytes_per_pixel == gpu_bytes_per_pixel) {
		...
		int gpu_channels = gpu_bytes_per_pixel /
						   VideoParams::get_bytes_per_channel(params.format());
		ByteArray tmp(width * height * gpu_bytes_per_pixel);
		memcpy(tmp.data(), mapped, static_cast<size_t>(tmp.size()));
		copy_pixels_with_channel_conversion(tmp.data(), data, width, height, 1,
										gpu_channels, params.channel_count(),
										params.format());
		if (linesize != width) {
			// Repack from tight CPU layout to caller's stride in-place.
			const char *tight_src = static_cast<const char *>(data);
			ByteArray tight(tight_src,
							tight_src + width * height * cpu_bytes_per_pixel);
			char *dst = static_cast<char *>(data);
			for (int row = 0; row < height; row++) {
				memcpy(dst + row * row_stride_bytes,
					   tight.data() + row * width * cpu_bytes_per_pixel,
					   static_cast<size_t>(width * cpu_bytes_per_pixel));
			}
		}
	}
...
void VulkanRenderer::clear_destination(olive::Texture *texture, double r,
									  double g, double b, double a)
{
	...
	if (texture) {
		uint64_t id = texture->id().value<uint64_t>();
		VulkanTexture *tex = map_lookup(textures_, id);
		if (!tex) {
			end_one_time_commands(cmd);
			return;
...
Color VulkanRenderer::get_pixel_from_texture(olive::Texture *texture,
										  const PointF &pt)
{
	if (!texture) {
		return Color();
	}
	int cpu_bytes_per_pixel = VideoParams::get_bytes_per_pixel(
		texture->format(), texture->channel_count());
	ByteArray data(cpu_bytes_per_pixel);
	...
	uint32_t px = static_cast<uint32_t>(
		std::min(std::max(0.0, pt.x()), double(tex->width - 1)));
	uint32_t py = static_cast<uint32_t>(
		std::min(std::max(0.0, pt.y()), double(tex->height - 1)));
	...
	VkBuffer staging_buffer;
	VkDeviceMemory staging_memory;
	if (!create_staging_buffer(gpu_bytes_per_pixel, &staging_buffer,
							 &staging_memory)) {
		return Color();
	}
	...
	copy_image_to_buffer(cmd, tex->image, staging_buffer, 1, 1, px, py);
	end_one_time_commands(cmd);
	...
	if (cpu_bytes_per_pixel == gpu_bytes_per_pixel) {
		...
		ByteArray gpu_pixel(gpu_bytes_per_pixel);
		memcpy(gpu_pixel.data(), mapped,
			   static_cast<size_t>(gpu_bytes_per_pixel));
		copy_pixels_with_channel_conversion(gpu_pixel.data(), data.data(), 1,
										1, 1, gpu_channels,
...
static bool is_sampler_type(const std::string &type)
{
	static const std::regex sampler_re(
		R"(sampler\d*D|samplerCube|sampler2DArray|sampler3D)");
	return std::regex_search(type, sampler_re);
}

// Ensures GLSL has a Vulkan-compatible version directive before shaderc compiles
// it as GLSL 450.
std::string VulkanRenderer::ensure_glsl_version450(const std::string &glsl) const
{
	std::string result = string_trimmed(glsl);
	if (string_starts_with(result, "#version")) {
		size_t end = result.find('\n');
		std::string rest =
			(end != std::string::npos) ? result.substr(end + 1) : std::string();
...
std::string VulkanRenderer::convert_glsl_to_vulkan(const std::string &glsl,
											VkShaderStageFlagBits stage)
{
	std::string result = glsl;

	// Olive uses the legacy texture2D/texture3D names in some shaders.
	string_replace_all(&result, "texture2D(", "texture(");
	string_replace_all(&result, "texture3D(", "texture(");

	// Ensure fragment shader output has layout
	if (stage == VK_SHADER_STAGE_FRAGMENT_BIT) {
		string_replace_all(&result, "out vec4 frag_color;",
						   "layout(location = 0) out vec4 frag_color;");
		string_replace_all(&result, "in vec2 ove_texcoord;",
						   "layout(location = 0) in vec2 ove_texcoord;");
	}

	// Add layout to vertex attributes and varyings
	if (stage == VK_SHADER_STAGE_VERTEX_BIT) {
		string_replace_all(&result, "in vec4 a_position;",
						   "layout(location = 0) in vec4 a_position;");
		string_replace_all(&result, "in vec2 a_texcoord;",
						   "layout(location = 1) in vec2 a_texcoord;");
		string_replace_all(&result, "out vec2 ove_texcoord;",
						   "layout(location = 0) out vec2 ove_texcoord;");
	}
...
VkDeviceSize VulkanRenderer::get_std140_size(const std::string &type) const
{
	if (type == ("float"))
		return 4;
	if (type == ("vec2"))
...
VkDeviceSize VulkanRenderer::get_std140_alignment(const std::string &type) const
{
	if (type == ("float"))
		return 4;
	if (type == ("vec2"))
...
void VulkanRenderer::extract_uniforms(const std::string &glsl,
									 std::vector<UniformInfo> *out_uniforms,
									 std::vector<std::string> *out_samplers) const
{
	for (const UniformDeclMatch &m : find_uniform_decls(glsl)) {
		const std::string &type = m.type;
		const std::string &name = m.name;

		if (is_sampler_type(type)) {
			if (out_samplers &&
				std::find(out_samplers->begin(), out_samplers->end(), name) ==
					out_samplers->end()) {
				out_samplers->push_back(name);
			}
...
void VulkanRenderer::compute_uniform_layout(std::vector<UniformInfo> *uniforms) const
{
	VkDeviceSize offset = 0;
	for (UniformInfo &info : *uniforms) {
		VkDeviceSize align = get_std140_alignment(info.type);
		offset = align_size(offset, align);
		info.offset = offset;
		info.size = get_std140_size(info.type);
		offset += info.size;
	}
}
...
std::string
VulkanRenderer::build_ubo_block(const std::vector<UniformInfo> &uniforms) const
{
	if (uniforms.empty()) {
		return std::string();
	}
...
std::string VulkanRenderer::rewrite_shader_with_ubo(
	const std::string &glsl, const std::vector<UniformInfo> &all_uniforms,
	const std::map<std::string, int> &sampler_bindings) const
{
	std::string result = glsl;
	...
	std::vector<UniformDeclMatch> matches = find_uniform_decls(result);
	for (auto it = matches.rbegin(); it != matches.rend(); ++it) {
		const UniformDeclMatch &m = *it;
		const std::string &type = m.type;
		const std::string &name = m.name;

		if (is_sampler_type(type)) {
			int binding = map_value_or(sampler_bindings, name, -1);
			if (binding >= 0) {
				std::string new_decl =
					"layout(set = 0, binding = " + std::to_string(binding) +
					") uniform " + type + " " + name + ";";
				result.replace(m.offset, m.length, new_decl);
			}
		} else {
			result.erase(m.offset, m.length);
		}
	}
	...
	for (const UniformInfo &info : all_uniforms) {
		result = std::regex_replace(result,
									std::regex("\\b" + info.name + "\\b"),
									"ubo." + info.name);
	}

	// Third pass: insert the shared UBO block after the #version line.
	if (!all_uniforms.empty()) {
		std::string ubo = build_ubo_block(all_uniforms);
		size_t version_end = result.find('\n');
		if (version_end != std::string::npos &&
			string_starts_with(result, "#version")) {
...
bool VulkanRenderer::compile_glsl_to_spv(const std::string &glsl,
									  VkShaderStageFlagBits stage,
									  ByteArray *out_spv)
{
#ifdef OAK_HAS_SHADERC
	...
	std::string converted = convert_glsl_to_vulkan(glsl, stage);

	shaderc_compilation_result_t compile_result = shaderc_compile_into_spv(
		compiler, converted.data(), converted.size(), kind,
	...
	(void) stage;
	(void) out_spv;
	fprintf(stderr, "shaderc not available, cannot compile GLSL to SPIR-V\n");
	return false;
#endif
}

// Converts, compiles, and stores a shader pair plus descriptor metadata.
Variant VulkanRenderer::create_native_shader(olive::ShaderCode code)
{
	std::lock_guard<std::mutex> lock(mutex_);
	...
	std::string vert_code = code.vert_code();
	std::string frag_code = code.frag_code();

	// Use default shaders if empty
	if (vert_code.empty()) {
		vert_code = FileFunctions::read_file_as_string(
			(":/shaders/default.vert"));
	}
	if (frag_code.empty()) {
		frag_code = FileFunctions::read_file_as_string(
			(":/shaders/default.frag"));
	}

	// Make sure both stages declare a Vulkan-compatible version.
	vert_code = ensure_glsl_version450(vert_code);
	frag_code = ensure_glsl_version450(frag_code);
	...
	std::vector<UniformInfo> vert_uniforms;
	std::vector<UniformInfo> frag_uniforms;
	std::vector<std::string> vert_samplers;
	std::vector<std::string> frag_samplers;
	extract_uniforms(vert_code, &vert_uniforms, &vert_samplers);
	extract_uniforms(frag_code, &frag_uniforms, &frag_samplers);

	std::vector<UniformInfo> all_uniforms = vert_uniforms;
	for (const UniformInfo &fu : frag_uniforms) {
		...
		if (!exists) {
			all_uniforms.push_back(fu);
		}
	}
	compute_uniform_layout(&all_uniforms);

	std::vector<std::string> all_samplers = vert_samplers;
	for (const std::string &name : frag_samplers) {
		if (std::find(all_samplers.begin(), all_samplers.end(), name) ==
			all_samplers.end()) {
			all_samplers.push_back(name);
		}
	}
	std::map<std::string, int> sampler_bindings;
	...
	std::string converted_vert =
		rewrite_shader_with_ubo(vert_code, all_uniforms, sampler_bindings);
	std::string converted_frag =
		rewrite_shader_with_ubo(frag_code, all_uniforms, sampler_bindings);

	converted_vert =
		convert_glsl_to_vulkan(converted_vert, VK_SHADER_STAGE_VERTEX_BIT);
	converted_frag =
		convert_glsl_to_vulkan(converted_frag, VK_SHADER_STAGE_FRAGMENT_BIT);

	if (!compile_glsl_to_spv(converted_vert, VK_SHADER_STAGE_VERTEX_BIT,
						  &vert_spv)) {
		fprintf(stderr, "Failed to compile Vulkan vertex shader:\n%s\n",
				converted_vert.c_str());
		return Variant();
	}
	if (!compile_glsl_to_spv(converted_frag, VK_SHADER_STAGE_FRAGMENT_BIT,
						  &frag_spv)) {
		fprintf(stderr, "Failed to compile Vulkan fragment shader:\n%s\n",
				converted_frag.c_str());
		return Variant();
	}
	...
	sh->ubo_size = 0;
	for (const UniformInfo &u : all_uniforms) {
		sh->ubo_size = std::max(sh->ubo_size, u.offset + u.size);
	}

	VkShaderModuleCreateInfo vert_info = {};
	vert_info.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
	vert_info.codeSize = static_cast<size_t>(vert_spv.size());
	vert_info.pCode = reinterpret_cast<const uint32_t *>(vert_spv.data());
	...
	frag_info.pCode = reinterpret_cast<const uint32_t *>(frag_spv.data());
	...
	return Variant::from_value(sh->id);
}

// Releases shader modules, descriptor layout, pipeline layout, and pipelines.
void VulkanRenderer::destroy_native_shader(Variant shader)
{
	std::lock_guard<std::mutex> lock(mutex_);
	uint64_t id = shader.value<uint64_t>();
	VulkanShader *sh = map_take(shaders_, id);
	if (!sh) {
		return;
	}
...
bool VulkanRenderer::create_pipeline_for_shader(VulkanShader *shader,
											 const VideoParams &dest_params,

											 VkFormat render_pass_format)
{
	if (shader->pipeline_cache.count(render_pass_format)) {
		return true;
	}
	...
	pipeline_info.renderPass = get_or_create_render_pass(render_pass_format, false);
	...
	if (result != VK_SUCCESS) {
		fprintf(stderr, "Failed to create Vulkan graphics pipeline: %d\n", result);
		return false;
	}
...
void VulkanRenderer::blit_pass(VulkanShader *shader, VulkanTexture *dest_tex,
							  const std::vector<TextureBinding> &bindings,
							  const ByteArray &ubo_data,
							  ...
							  bool clear_destination, int iteration)
{
	...
	VkRenderPass render_pass =
		get_or_create_render_pass(render_pass_format, clear_destination);
	if (render_pass == VK_NULL_HANDLE) {
		return;
	}

	if (!create_pipeline_for_shader(shader, destination_params,
								 render_pass_format)) {
		return;
	}
	...
	{
		auto pipeline_it = shader->pipeline_cache.find(render_pass_format);
		if (pipeline_it != shader->pipeline_cache.end()) {
	...
	if (dest_tex->framebuffer == VK_NULL_HANDLE) {
		...
		fb_info.renderPass = get_or_create_render_pass(render_pass_format, false);
		...
		if (fb_result != VK_SUCCESS) {
			fprintf(stderr, "Failed to create Vulkan framebuffer: %d\n", fb_result);
			return;
		}
	}
	...
	if (shader->ubo_size > 0 && !ubo_data.empty()) {
		if (create_staging_buffer(shader->ubo_size, &ubo_buffer, &ubo_memory)) {
			void *mapped;
			vkMapMemory(device_, ubo_memory, 0, shader->ubo_size, 0, &mapped);
			memcpy(mapped, ubo_data.data(),
	...
	bool descriptors_needed = (shader->ubo_size > 0 || !bindings.empty());
	if (descriptors_needed) {
		if (descriptor_sets_since_reset_ >= k_max_descriptor_sets - 16) {
		...
		if (result != VK_SUCCESS) {
			fprintf(stderr, "Failed to allocate Vulkan descriptor set: %d\n", result);
			if (ubo_buffer != VK_NULL_HANDLE) {
				destroy_staging_buffer(ubo_buffer, ubo_memory);
			}
			return;
		}
		...
		if (!bindings.empty()) {
			image_infos.reserve(std::min(bindings.size(), static_cast<size_t>(16)));
			for (int i = 0; i < bindings.size() && i < 16; i++) {
				const TextureBinding &tb = bindings.at(i);
				VkDescriptorImageInfo img_info = {};
				img_info.sampler = get_sampler(tb.interp);
				...
				int binding = map_value_or(shader->sampler_bindings, tb.name, -1);
				if (binding < 0) {
					binding = 1 + i;
				}
		...
		if (!writes.empty()) {
			vkUpdateDescriptorSets(device_, writes.size(), writes.data(),
								   0, nullptr);
		}
	}
	...
	if (dest_tex->current_layout != VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL) {
		transition_image_layout(cmd, dest_tex->image, dest_tex->current_layout,
							  VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
	...
	for (const TextureBinding &tb : bindings) {
		if (tb.tex && tb.tex->current_layout !=
						  VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL) {
			transition_image_layout(cmd, tb.tex->image, tb.tex->current_layout,
								  VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
	...
	transition_image_layout(cmd, dest_tex->image,
						  VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
						  VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
...
void VulkanRenderer::blit(Variant shader_variant, olive::AcceleratedJob &a_job,
						  ...
						  bool clear_destination)
{
	...
	uint64_t shader_id = shader_variant.value<uint64_t>();
	...
	if (real_iteration_count > 1) {
		output_tex.texture = create_texture(destination_params);
		if (real_iteration_count > 2) {
			input_tex.texture = create_texture(destination_params);
		}
	}

	if (!destination) {
		final_tex.texture = create_texture(destination_params);
	}

	std::lock_guard<std::mutex> lock(mutex_);

	VulkanShader *shader = map_lookup(shaders_, shader_id);
	if (!shader) {
		return;
	}
	...
	if (destination) {
		uint64_t dest_id = destination->id().value<uint64_t>();
		dest_tex = map_lookup(textures_, dest_id);
		if (!dest_tex) {
			return;
		}
	} else {
		uint64_t final_id = final_tex.texture->id().value<uint64_t>();
		final_tex.native = map_lookup(textures_, final_id);
		if (!final_tex.native) {
			fprintf(stderr, "VulkanRenderer::Blit failed to resolve temporary destination texture\n");
			return;
		}
	...
	if (output_tex.texture) {
		uint64_t id = output_tex.texture->id().value<uint64_t>();
		output_tex.native = map_lookup(textures_, id);
	}
	if (input_tex.texture) {
		uint64_t id = input_tex.texture->id().value<uint64_t>();
		input_tex.native = map_lookup(textures_, id);
	}

	// Collect textures to bind and build base UBO data
	std::vector<TextureBinding> base_bindings;
	ByteArray base_ubo_data;
	if (shader->ubo_size > 0) {
		base_ubo_data.resize(static_cast<int>(shader->ubo_size));
		std::fill(base_ubo_data.begin(), base_ubo_data.end(), 0);
	}

	for (const auto &pair : job.get_values()) {
		const NodeValue &value = pair.second;
		if (value.type() == NodeValue::k_texture) {
			TexturePtr texture = value.to_texture();
			VulkanTexture *vtex = nullptr;
			if (texture) {
				uint64_t tid = texture->id().value<uint64_t>();
				vtex = map_lookup(textures_, tid);
			}
			base_bindings.push_back(
				{ pair.first, vtex, job.get_interpolation(pair.first) });
		} else if (!shader->uniforms.empty() && shader->ubo_size > 0) {
			// Find matching uniform
			for (const UniformInfo &u : shader->uniforms) {
				if (u.name != pair.first)
					continue;
				char *dst = base_ubo_data.data() + static_cast<int>(u.offset);
				switch (value.type()) {
				case NodeValue::k_float:
					*reinterpret_cast<float *>(dst) =
						static_cast<float>(value.to_double());
					break;
				...
				case NodeValue::k_boolean:
					*reinterpret_cast<int *>(dst) = value.to_bool() ? 1 : 0;
					break;
				...
				case NodeValue::k_matrix: {
					Matrix4x4 m = value.to_matrix();
					memcpy(dst, m.const_data(), sizeof(float) * 16);
					break;
				}
				case NodeValue::k_color: {
					Color c = value.to_color();
				...
				case NodeValue::k_combo:
					*reinterpret_cast<int *>(dst) = value.to_int();
					break;
				default:
					break;
				}
	...
	if (shader->ubo_size > 0) {
		for (const UniformInfo &u : shader->uniforms) {
			char *dst = base_ubo_data.data() + static_cast<int>(u.offset);
			if (u.name == ("ove_mvpmat")) {
				Matrix4x4 m = job.get(("ove_mvpmat")).to_matrix();
				memcpy(dst, m.const_data(), sizeof(float) * 16);
			} else if (u.name == ("ove_cropmatrix")) {
				Matrix4x4 m =
					job.get(("ove_cropmatrix")).to_matrix();
				memcpy(dst, m.const_data(), sizeof(float) * 16);
			} else if (u.name == ("ove_maintex_alpha")) {
				*reinterpret_cast<int *>(dst) =
					job.get(("ove_maintex_alpha")).to_int();
			} else if (u.name == ("ove_force_opaque")) {
				*reinterpret_cast<int *>(dst) =
					job.get(("ove_force_opaque")).to_bool() ? 1 :
																		   0;
			}
		}
	}
	...
	if (shader->ubo_size > 0) {
		for (const TextureBinding &tb : base_bindings) {
			std::string enabled_name = tb.name + ("_enabled");
			for (const UniformInfo &u : shader->uniforms) {
	...
	for (int iteration = 0; iteration < real_iteration_count; ++iteration) {
		std::vector<TextureBinding> pass_bindings = base_bindings;
		ByteArray pass_ubo_data = base_ubo_data;

		// Set iteration number
		if (shader->ubo_size > 0) {
			for (const UniformInfo &u : shader->uniforms) {
				if (u.name == ("ove_iteration")) {
					char *dst =
		...
		blit_pass(shader, pass_dest, pass_bindings, pass_ubo_data,
