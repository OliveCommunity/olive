...
private:
	...
	bool compile_glsl_to_spv(const std::string &glsl, VkShaderStageFlagBits stage,
						  ByteArray *out_spv);
	// Rewrites an Oak GLSL shader into Vulkan-compatible GLSL.
	std::string convert_glsl_to_vulkan(const std::string &glsl,
									VkShaderStageFlagBits stage);
	// Ensures a shader declares a Vulkan-compatible GLSL version.
	std::string ensure_glsl_version450(const std::string &glsl) const;
	// Extracts uniforms and sampler names from GLSL declarations.
	void extract_uniforms(const std::string &glsl,
	...
	void compute_uniform_layout(std::vector<UniformInfo> *uniforms) const;
	// Builds the generated uniform block used by rewritten shaders.
	std::string build_ubo_block(const std::vector<UniformInfo> &uniforms) const;
	// Rewrites standalone uniforms and samplers into explicit UBO/sampler
	// bindings accepted by Vulkan GLSL.
	std::string
	rewrite_shader_with_ubo(const std::string &glsl,
						 const std::vector<UniformInfo> &all_uniforms,
						 const std::map<std::string, int> &sampler_bindings) const;
	// Returns std140 storage size for a supported GLSL type.
	VkDeviceSize get_std140_size(const std::string &type) const;
	// Returns std140 alignment for a supported GLSL type.
	VkDeviceSize get_std140_alignment(const std::string &type) const;
	...
	std::map<uint64_t, VkRenderPass> render_pass_cache_;
	...
	std::map<uint64_t, VulkanTexture *> textures_;

	// Shader handle counter
	uint64_t next_shader_id_ = 1;
	std::map<uint64_t, VulkanShader *> shaders_;
