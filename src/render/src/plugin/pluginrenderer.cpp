...
// NodeValueRow is a std::map with no defaulting operator[]; this mirrors the
// original `values.value(key)` (missing key yields a default NodeValue).
static olive::NodeValue
lookup_value(const olive::NodeValueRow &values, const std::string &key)
{
	auto it = values.find(key);
	return it != values.end() ? it->second : olive::NodeValue();
}
					...
					int *bytes_per_pixel)
{
	...
	int pix_fmt =
		olive::FFmpegUtils::get_ffmpeg_pixel_format(pixel_format, channel_count);
								...
								const olive::NodeValueRow &values, OfxTime time)
{
	...
	for (const auto &entry : params) {
		...
		const olive::NodeValue &value = value_it->second;
		...
		if (type == kOfxParamTypeInteger) {
			if (auto *param = dynamic_cast<OFX::Host::Param::IntegerInstance *>(
					entry.second)) {
				param->set(time, value.data().to_int());
			}
			continue;
		}
		if (type == kOfxParamTypeDouble) {
			if (auto *param = dynamic_cast<OFX::Host::Param::DoubleInstance *>(
					entry.second)) {
				double v = value.data().to_double();
				if (std::isnan(v) || std::isinf(v)) {
					fprintf(stderr, "[PLUGIN] NaN/Inf in double param %s replacing with default\n", key.c_str());
					auto *default_prop =
		...
		if (type == kOfxParamTypeBoolean) {
			if (auto *param = dynamic_cast<OFX::Host::Param::BooleanInstance *>(
					entry.second)) {
				param->set(time, value.data().to_bool());
			}
			continue;
		}
		...
		if (type == kOfxParamTypeString || type == kOfxParamTypeCustom ||
			...
			if (auto *param = dynamic_cast<OFX::Host::Param::StringInstance *>(
					entry.second)) {
				const std::string utf8 = value.data().to_string();
				param->set(time, utf8.c_str());
			}
			continue;
		}
		if (type == kOfxParamTypeRGBA) {
			if (auto *param = dynamic_cast<OFX::Host::Param::RGBAInstance *>(
					entry.second)) {
				if (value.data().can_convert<olive::core::Color>()) {
					const auto c = value.data().value<olive::core::Color>();
					param->set(time, c.red(), c.green(), c.blue(), c.alpha());
				} else if (value.data().can_convert<olive::Vector4D>()) {
					const olive::Vector4D v = value.data().value<olive::Vector4D>();
					param->set(time, v.x(), v.y(), v.z(), v.w());
				} else if (value.data().can_convert<olive::Vector3D>()) {
					const olive::Vector3D v = value.data().value<olive::Vector3D>();
		...
		if (type == kOfxParamTypeDouble2D) {
			if (auto *param =
					dynamic_cast<OFX::Host::Param::Double2DInstance *>(
						entry.second)) {
				if (value.data().can_convert<olive::Vector2D>()) {
					const olive::Vector2D v = value.data().value<olive::Vector2D>();
		...
		if (type == kOfxParamTypeDouble3D) {
			if (auto *param =
					dynamic_cast<OFX::Host::Param::Double3DInstance *>(
						entry.second)) {
				if (value.data().can_convert<olive::Vector3D>()) {
					const olive::Vector3D v = value.data().value<olive::Vector3D>();
					param->set(time, v.x(), v.y(), v.z());
				}
			}
...
// 作用：读取 clip 偏好（像素深度与分量）并更新 olive::VideoParams。
// Purpose: Apply clip preferences (depth/components) into olive::VideoParams.
...
// 作用：将 OFX bit depth 字符串映射为内部 olive::PixelFormat。
// Purpose: Map OFX bit depth string to internal olive::PixelFormat.
...
// 作用：将内部 olive::PixelFormat 转为 OFX bit depth 字符串。
// Purpose: Map internal olive::PixelFormat to OFX bit depth string.
...
create_avframe_from_ofx_image(OFX::Host::ImageEffect::Image &image)
{
	...
	if (!data_ptr) {
		fprintf(stderr, "OFX output image missing data pointer\n");
		return nullptr;
	}
	...
	if (width <= 0 || height <= 0) {
		fprintf(stderr, "OFX output image has invalid bounds %d %d %d %d\n", bounds[0], bounds[1], bounds[2], bounds[3]);
		return nullptr;
	}
	...
	if (pix_fmt == fb_pix_fmt_none || bytes_per_pixel <= 0) {
		fprintf(stderr, "OFX output image has unsupported pixel format depth=%s components=%s\n", image.getStringProperty(kOfxImageEffectPropPixelDepth).c_str(), image.getStringProperty(kOfxImageEffectPropComponents).c_str());
		return nullptr;
	}
...
// 前置声明：必要时将 AVFrame 转换为目标 olive::VideoParams 对应格式。
// Forward declaration: Convert AVFrame to match destination olive::VideoParams when needed.

// 作用：按指定 olive::VideoParams 复制 OFX Image 到 AVFrame，必要时做格式转换。
// Purpose: Copy OFX Image data into an AVFrame using target olive::VideoParams with format conversion.
static olive::AVFramePtr
	...
	if (needs_conversion) {
		...
		if (src_fmt != fb_pix_fmt_none) {
			...
			if (src_frame->get_buffer(0) >= 0) {
				...
				fprintf(stderr, "[WARN] av_frame_get_buffer failed for src_fmt=%d\n", src_fmt);
			}
		} else {
			fprintf(stderr, "[WARN] src_fmt is NONE for depth=%s\n", image_depth.c_str());
		}
	}
...
// 作用：将 olive::VideoParams 映射为最终输出的 AVPixelFormat。
// Purpose: Map olive::VideoParams to the final AVPixelFormat.
static int
get_destination_av_pixel_format(const olive::VideoParams &params)
{
	int pix_fmt = olive::FFmpegUtils::get_ffmpeg_pixel_format(
...
// 作用：将 AVPixelFormat 映射为 Olive 的 olive::PixelFormat 和通道数（仅常见 packed 格式）。
...
// 作用：必要时将 AVFrame 转换为目标 olive::VideoParams 对应格式。
					 ...
					 olive::Renderer *renderer = nullptr)
{
	...
	if (dst->get_buffer(0) < 0) {
		fprintf(stderr, "[WARN] av_frame_get_buffer failed for dst_fmt=%d\n", dst_fmt);
		return src;
	}
				...
				if (dst_tex) {
					...
					job.insert("ove_maintex",
							   olive::NodeValue(olive::NodeValue::k_texture,
												olive::Variant::from_value(src_tex)));
	...
	fprintf(stderr, "[WARN] ConvertFrameIfNeeded failed (sws_scale + GPU both unavailable). Returning unconverted source. src_fmt=%d dst_fmt=%d\n", src->format(), dst_fmt);
	return src;
}
...
// 作用：将纹理转换为指定 olive::VideoParams。优先使用 GPU shader 做格式转换，
						  ...
						  const OFX::Host::ImageEffect::Instance *instance)
{
	...
	fprintf(stderr, "OFX action failed: %s plugin=%s status=%s (%d)\n", action, plugin_id_for_instance(instance).c_str(), OFX::StatStr(stat), stat);
}

// 作用：输出 clip 的声明属性与关联 olive::VideoParams，辅助定位格式不一致。
// Purpose: Log clip declared properties and olive::VideoParams for debugging.
static void log_clip_state(const char *label,
						 const OFX::Host::ImageEffect::ClipInstance *clip,
						 const olive::VideoParams *params)
{
	if (!clip) {
		fprintf(stderr, "OFX clip state %s <null>\n", label);
		return;
	}
	fprintf(stderr, "OFX clip state %s name=%s pixelDepth=%s components=%s\n", label, clip->getName().c_str(), clip->getPixelDepth().c_str(), clip->getComponents().c_str());
	if (params) {
		fprintf(stderr, "OFX clip params %s width=%d height=%d format=%d channels=%d\n", label, params->width(), params->height(), static_cast<int>(params->format()), params->channel_count());
	}
}
...
{
	if (!image) {
		/*fprintf(stderr, "OFX image props %s <null>\n", label);
		return;*/
	}
	int bounds[4] = { 0, 0, 0, 0 };
	int rod[4] = { 0, 0, 0, 0 };
	...
	[[maybe_unused]] const std::string &components =
		image->getStringProperty(kOfxImageEffectPropComponents);
	/*fprintf(stderr, "OFX image props %s pixelDepth=%s components=%s rowBytes=%d bounds=%d %d %d %d rod=%d %d %d %d\n", label, depth.c_str(), components.c_str(), row_bytes, bounds[0], bounds[1], bounds[2], bounds[3], rod[0], rod[1], rod[2], rod[3]);*/
}
...
/// Deliver a user-visible plugin error. The original showed a QMessageBox and
/// undid the last operation on the GUI thread; UI interaction is an app-layer
/// responsibility, so the message is handed to the facade-installed callback.
/// With no callback installed this is log-only (mirrors the original behavior
/// when no QCoreApplication existed).
static void schedule_error_dialog_and_undo(
	const olive::plugin::PluginRenderer::ErrorCallback &callback,
...
void olive::plugin::PluginRenderer::render_plugin(
	olive::TexturePtr src, olive::plugin::PluginJob &job,
	...
	bool clear_destination, bool interactive)
{
	...
	const bool use_opengl =
		...
		destination->renderer() == renderer_ && !destination->id().is_null();
	if (olive_instance) {
	...
	auto is_usable_input = [](const olive::TexturePtr &tex) {
		if (!tex) {
			return false;
		}
	...
	std::map<std::string, olive::TexturePtr> input_textures;
	...
	for (const auto &entry : clips) {
		...
		const std::string &clip_key = entry.first;
		olive::TexturePtr input_tex = nullptr;
		if (!effect_input_id.empty() && clip_key == effect_input_id &&
			is_usable_input(src)) {
			input_tex = src;
		} else {
			input_tex = lookup_value(values, clip_key).to_texture();
			if (!input_tex &&
				entry.first == kOfxImageEffectSimpleSourceClipName) {
				input_tex = lookup_value(values, k_texture_input).to_texture();
			}
		}
	...
	try {
		...
		fprintf(stderr, "OFX getClipPreferences threw exception for plugin=%s stat=%d\n", plugin_id_for_instance(instance).c_str(), e.getStatus());
		mark_render_failure(destination);
		schedule_error_dialog_and_undo(
			error_callback_,
			"Plugin " + plugin_id_for_instance(instance) +
				" failed because connected inputs have different frame rates.\n"
				"The last operation has been undone.");
		return;
	} catch (const std::exception &e) {
		fprintf(stderr, "OFX getClipPreferences threw exception for plugin=%s what=%s\n", plugin_id_for_instance(instance).c_str(), e.what());
		mark_render_failure(destination);
		schedule_error_dialog_and_undo(
			error_callback_,
			"Plugin " + plugin_id_for_instance(instance) +
	...
	if (!ok) {
		fprintf(stderr, "OFX getClipPreferences failed for plugin=%s\n", plugin_id_for_instance(instance).c_str());
		mark_render_failure(destination);
		schedule_error_dialog_and_undo(
			error_callback_,
			"Plugin " + plugin_id_for_instance(instance) +
				" failed to get clip preferences.\n"
				"The last operation has been undone.");
		return;
	}
	...
	for (const auto &entry : input_clips) {
		...
		olive::TexturePtr input_tex = input_textures[entry.first];
		if (is_usable_input(input_tex)) {
			...
			olive::PixelFormat chosen_fmt = select_best_plugin_input_format(descriptor);
			olive::VideoParams params = input_tex->params();

			if (chosen_fmt != PixelFormat::invalid &&
				params.format() != chosen_fmt) {
				params.set_format(chosen_fmt);
				olive::TexturePtr converted_tex =
	...
	if (stat != kOfxStatOK && stat != kOfxStatReplyDefault) {
		...
		if (stat == kOfxStatErrBadHandle) {
			fprintf(stderr, "OFX getRegionOfInterest returned BadHandle for plugin=%s - using default RoI\n", plugin_id_for_instance(instance).c_str());
		} else {
	...
	olive::PixelFormat best_fmt = select_best_plugin_input_format(descriptor);
	...
	if (!output_params.is_valid()) {
		fprintf(stderr, "OFX render skipped due to invalid output params for plugin=%s\n", plugin_id_for_instance(instance).c_str());
		mark_render_failure(destination);
	...
	if (!use_opengl) {
		...
		if (!output_image) {
			fprintf(stderr, "OFX getOutputImage returned null for plugin=%s\n", plugin_id_for_instance(instance).c_str());
			mark_render_failure(destination);
		...
		if (!destination || destination->id().is_null()) {
#ifdef OFX_SUPPORTS_OPENGLRENDER
	...
	if (!use_opengl) {
		...
		if (!frame_ptr) {
			fprintf(stderr, "OFX output image conversion failed for plugin=%s\n", plugin_id_for_instance(instance).c_str());
			instance->endRenderAction(frame, num_frames_to_render, 1.0,
									  interactive, render_scale, true,
									  interactive);
			return;
		...
		if (destination->renderer() && converted && converted->data(0) &&
			...
			fprintf(stderr, "OFX output pixel format mismatch for plugin=%s\n", plugin_id_for_instance(instance).c_str());
		}
	} else {
