...
use oakundo::error::{
    OAKUNDO_E_INVALID, OAKUNDO_E_NOT_FOUND, OAKUNDO_OK,
};
...
fn multi_redo_undo_ordering() {
	...
	assert_eq!(unsafe { oakundo_command_multi_add_child(multi, ca) }, OAKUNDO_OK);
	assert_eq!(unsafe { oakundo_command_multi_add_child(multi, cb) }, OAKUNDO_OK);
	assert_eq!(unsafe { oakundo_command_multi_add_child(multi, cc) }, OAKUNDO_OK);

	// child_count reflects three.
	let mut count: c_int = 0;
	assert_eq!(unsafe { oakundo_command_multi_child_count(multi, &mut count) }, OAKUNDO_OK);
	assert_eq!(count, 3);
	...
	assert_eq!(unsafe { oakundo_command_multi_child(multi, 0, &mut child0) }, OAKUNDO_OK);
	...
	assert_eq!(events(&trace).iter().filter(|e| e.starts_with("free:")).count(), 3);
}
...
fn stack_push_undo_redo_queries() {
	...
	assert_eq!(unsafe { oakundo_undostack_count(stack, &mut count) }, OAKUNDO_OK);
	assert_eq!(count, 1);
	assert_eq!(unsafe { oakundo_undostack_can_undo(stack, &mut value) }, OAKUNDO_OK);
	assert_eq!(value, 0);
	assert_eq!(unsafe { oakundo_undostack_can_redo(stack, &mut value) }, OAKUNDO_OK);
	assert_eq!(value, 0);
...
fn stack_jump_clamps() {
	...
	let mut index: i64 = 0;
	assert_eq!(unsafe { oakundo_undostack_index(stack, &mut index) }, OAKUNDO_OK);
	assert_eq!(index, 4);
...
fn command_redo_undo_now_null_is_invalid() {
	assert_eq!(unsafe { oakundo_command_redo_now(empty_cmd()) }, OAKUNDO_E_INVALID);
	assert_eq!(unsafe { oakundo_command_undo_now(empty_cmd()) }, OAKUNDO_E_INVALID);
}
...
fn command_multi_add_child_errors() {
	...
	assert_eq!(unsafe { oakundo_command_multi_add_child(empty_cmd(), child) }, OAKUNDO_E_INVALID);
	// child is null → E_INVALID (nothing taken).
	assert_eq!(unsafe { oakundo_command_multi_add_child(multi, empty_cmd()) }, OAKUNDO_E_INVALID);
	// target is a vtable command, not a multi → E_INVALID.
	assert_eq!(unsafe { oakundo_command_multi_add_child(vtable_cmd, child) }, OAKUNDO_E_INVALID);
...
fn command_multi_child_count_errors() {
	...
	assert_eq!(unsafe { oakundo_command_multi_child_count(multi, std::ptr::null_mut()) }, OAKUNDO_E_INVALID);
	// null multi → E_INVALID.
	assert_eq!(unsafe { oakundo_command_multi_child_count(empty_cmd(), &mut out) }, OAKUNDO_E_INVALID);
	// non-multi command → E_INVALID.
	assert_eq!(unsafe { oakundo_command_multi_child_count(vtable_cmd, &mut out) }, OAKUNDO_E_INVALID);
...
fn command_multi_child_errors() {
	...
	// null out pointer → E_INVALID.
	assert_eq!(unsafe { oakundo_command_multi_child(multi, 0, std::ptr::null_mut()) }, OAKUNDO_E_INVALID);
	// null multi → E_INVALID.
	assert_eq!(unsafe { oakundo_command_multi_child(empty_cmd(), 0, &mut out) }, OAKUNDO_E_INVALID);
	// non-multi command → E_INVALID.
	assert_eq!(unsafe { oakundo_command_multi_child(vtable_cmd, 0, &mut out) }, OAKUNDO_E_INVALID);
	// empty multi, negative index → E_NOT_FOUND.
	assert_eq!(unsafe { oakundo_command_multi_child(multi, -1, &mut out) }, OAKUNDO_E_NOT_FOUND);
	// empty multi, positive OOB → E_NOT_FOUND.
	assert_eq!(unsafe { oakundo_command_multi_child(multi, 5, &mut out) }, OAKUNDO_E_NOT_FOUND);
...
fn command_multi_child_success_borrowed() {
	...
	assert_eq!(unsafe { oakundo_command_multi_child(multi, 0, &mut child) }, OAKUNDO_OK);
...
fn undostack_push_errors() {
	...
	assert_eq!(unsafe { oakundo_undostack_push(stack, empty_cmd(), name.as_ptr()) }, OAKUNDO_E_INVALID);
	// empty stack → E_INVALID; command NOT drained (still owns its value).
	assert_eq!(unsafe { oakundo_undostack_push(empty_stack(), cmd, name.as_ptr()) }, OAKUNDO_E_INVALID);
	assert_eq!(events(&trace), Vec::<String>::new(), "no callbacks ran on failed push");

	// NULL name is accepted as an empty label → OK.
	assert_eq!(unsafe { oakundo_undostack_push(stack, cmd, std::ptr::null()) }, OAKUNDO_OK);
...
fn undostack_undo_redo_errors() {
	let mut stack = unsafe { new_stack() };
	assert_eq!(unsafe { oakundo_undostack_undo(empty_stack()) }, OAKUNDO_E_INVALID);
	assert_eq!(unsafe { oakundo_undostack_redo(empty_stack()) }, OAKUNDO_E_INVALID);
...
fn undostack_jump_clear_errors() {
	let mut stack = unsafe { new_stack() };
	assert_eq!(unsafe { oakundo_undostack_jump(empty_stack(), 3) }, OAKUNDO_E_INVALID);
	assert_eq!(unsafe { oakundo_undostack_clear(empty_stack()) }, OAKUNDO_E_INVALID);
...
fn undostack_push_drops_redoable_tail_via_ffi() {
	...
	let nc = CString::new("C").unwrap();
	let mut cc = unsafe { make_cmd(&mut probes[2] as *mut Probe) };
	assert_eq!(unsafe { oakundo_undostack_push(stack, cc, nc.as_ptr()) }, OAKUNDO_OK);
