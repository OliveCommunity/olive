	...
	const VkDebugUtilsMessengerCallbackDataEXT *p_callback_data, void *p_user_data)
{
	...
	if (message_severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT) {
		qWarning() << "Vulkan validation error:" << p_callback_data->pMessage;
	} else if (message_severity &
			   VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT) {
		qWarning() << "Vulkan validation warning:" << p_callback_data->pMessage;
	}
...
QVariant VulkanRenderer::create_native_texture(int width, int height, int depth,
											 PixelFormat format,
											 int channel_count,
	...
	if (data) {
		...
		if (create_staging_buffer(image_size, &staging_buffer, &staging_memory)) {
			...
			if (cpu_bytes_per_pixel == gpu_bytes_per_pixel) {
				...
				copy_pixels_with_channel_conversion(tmp.constData(), mapped, width,
...
void VulkanRenderer::destroy_native_texture(QVariant texture)
{
	QMutexLocker lock(&mutex_);
...
void VulkanRenderer::upload_to_texture(const QVariant &handle,
									 ...
									 const void *data, int linesize)
{
	...
	if (cpu_bytes_per_pixel == gpu_bytes_per_pixel) {
		...
		copy_pixels_with_channel_conversion(tmp.constData(), mapped, width, height,
...
void VulkanRenderer::download_from_texture(const QVariant &handle,
										 ...
										 int linesize)
{
	...
	if (cpu_bytes_per_pixel == gpu_bytes_per_pixel) {
		...
		copy_pixels_with_channel_conversion(tmp.constData(), data, width, height, 1,
										  ...
										  const QPointF &pt)
{
	...
	if (cpu_bytes_per_pixel == gpu_bytes_per_pixel) {
		...
		copy_pixels_with_channel_conversion(gpu_pixel.constData(), data.data(), 1,
										1, 1, gpu_channels,
...
static bool is_sampler_type(const QString &type)
{
...
// it as GLSL 450.
QString VulkanRenderer::ensure_glsl_version450(const QString &glsl) const
{
...
QString VulkanRenderer::convert_glsl_to_vulkan(const QString &glsl,
...
VkDeviceSize VulkanRenderer::get_std140_size(const QString &type) const
{
...
VkDeviceSize VulkanRenderer::get_std140_alignment(const QString &type) const
{
	if (type == QStringLiteral("float"))
		return 4;
	if (type == QStringLiteral("vec2"))
...
void VulkanRenderer::extract_uniforms(const QString &glsl,
...
void VulkanRenderer::compute_uniform_layout(QVector<UniformInfo> *uniforms) const
{
...
QString
VulkanRenderer::build_ubo_block(const QVector<UniformInfo> &uniforms) const
{
	if (uniforms.isEmpty()) {
		return QString();
	}
...
QString VulkanRenderer::rewrite_shader_with_ubo(
	...
	const QHash<QString, int> &sampler_bindings) const
{
	...
	if (!all_uniforms.isEmpty()) {
		QString ubo = build_ubo_block(all_uniforms);
...
bool VulkanRenderer::compile_glsl_to_spv(const QString &glsl,
									  VkShaderStageFlagBits stage,
									  QByteArray *out_spv)
{
#ifdef OAK_HAS_SHADERC
	...
	QString converted = convert_glsl_to_vulkan(glsl, stage);
...
QVariant VulkanRenderer::create_native_shader(olive::ShaderCode code)
{
	QMutexLocker lock(&mutex_);
...
void VulkanRenderer::destroy_native_shader(QVariant shader)
{
	QMutexLocker lock(&mutex_);
	...
	for (auto it = job.get_values().constBegin();
		...
		if (value.type() == NodeValue::k_texture) {
			...
			base_bindings.append(
				{ it.key(), vtex, job.get_interpolation(it.key()) });
	...
	if (shader->ubo_size > 0) {
		for (const UniformInfo &u : shader->uniforms) {
			...
			if (u.name == QStringLiteral("ove_mvpmat")) {
				QMatrix4x4 m = job.get(QStringLiteral("ove_mvpmat")).to_matrix();
				memcpy(dst, m.constData(), sizeof(float) * 16);
				...
				*reinterpret_cast<int *>(dst) =
					job.get(QStringLiteral("ove_maintex_alpha")).to_int();
			} else if (u.name == QStringLiteral("ove_force_opaque")) {
				*reinterpret_cast<int *>(dst) =
					job.get(QStringLiteral("ove_force_opaque")).to_bool() ? 1 :
																		   0;
			}
		}
	}
